...
The constants the package reads off these results (`FEW_LINEAGES`,
`KURTOSIS_LIMIT`, `DEFAULT_MIN_SHARED_SUPPORT`, `CENSORED_GROUP_LIMIT`,
`SINGLE_CUT_PREVALENCE`) are checked against them before step 2; a change of
a constant is a new commit, and step 1 is repeated on it. The support gate of
the decomposition is read from the decomposition study by
`summarize_model_checks.py gate`, the last line of `commands/summaries.txt`.
...
$S --array=0-58  --cpus-per-task=48 --mem=180G --time=24:00:00 $C/lines.sbatch $C/commands/mic_panels_packed.txt
```

Each line of a `_packed` command file runs 48 replicate chunks at once, one per
core, and fails if any of them fails; `make_population_commands.py` and
`make_check_commands.py` write them. `mic_panels_packed.txt` holds the
multi-panel designs and the plasmode designs on the recorded *S. suis* design
(`mic_inference/calibrate_panels.py`); it reads the empirical inputs.
...
$S --array=0-162 --cpus-per-task=48 --mem=96G --time=24:00:00 $C/lines.sbatch $C/commands/population_mixing_packed.txt
$S --array=0-16  --cpus-per-task=48 --mem=96G --time=24:00:00 $C/lines.sbatch $C/commands/mic_mixture_packed.txt
```

The last two arrays read the critical value: the mixing check of the
population model on the validation and robustness cells, and the
wild-type / non-wild-type mixture designs of `mic_inference/calibrate_mixture.py`.

The check of the Gaussian MIC model (`mic_inference/shape_check.py`) is run on
the datasets of the MIC null, extension and plasmode arrays, drawn by the same
generators from the same seeds, so that the summary joins each checked dataset
to its coverage row:

```bash
$S --array=0-132 --cpus-per-task=48 --mem=180G --time=24:00:00 $C/lines.sbatch $C/commands/mic_shape_packed.txt
```
