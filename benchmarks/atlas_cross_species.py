...
WHAT THIS IS FOR. The worked examples are one species, one
...
def run_species(raw: Path, organism: str) -> dict:
    ...
    for agent in agents:
        ...
        screen.record["n_conflicting_calls"] = sum(
            agent in r and not np.isfinite(r[agent]) for r in rows)
        ...
        out["agents"][agent] = {
            ...
            "n_conflicting_calls": screen.record["n_conflicting_calls"],
    ...
    for record in results:
        for agent, cell in (record.get("agents") or {}).items():
            ...
            row = {"species": record["organism"], "agent": agent,
                   ...
                   # The estimator states, per cell, whether the cohort can
                   # support the estimate at all: the support is the share of
                   # isolates in lineages of two or more, and below 0.90 the
                   # measured coverage of the interval is not the nominal one.
                   # Leaving the flag out of the table let a number the
                   # package had refused be read as one it had returned.
                   "support": cell["support"],
                   "estimable": bool(cell["estimable"])}
...
def _layer(cells: list[dict]) -> dict:
    """The reading of one set of cells, for the admitted set and for all."""
    if not cells:
        return {"cells": 0, "species": 0, "kappa_median": None,
                "kappa_permuted_median": None, "cells_ci_excludes_zero": 0}
    return {
        "cells": len(cells),
        "species": len({c["species"] for c in cells}),
        ...
        "kappa_permuted_mean": statistics.fmean(
            c["kappa_permuted"] for c in cells),
        "cells_ci_excludes_zero": sum(1 for c in cells if c["ci_low"] > 0)}
...
def summarise(table: list[dict], results: list[dict]) -> dict:
    ...
    admitted = [r for r in table if r.get("estimable")]
    per_species = {}
    for record in results:
        cells = [r for r in table if r["species"] == record["organism"]]
        if not cells:
            continue
        adm = [c for c in cells if c.get("estimable")]
        per_species[record["organism"]] = {
            ...
            "kappa_permuted_median": statistics.median(
                c["kappa_permuted"] for c in cells),
            # The same species read on the cells its cohort can support. A
            # species with none is reported with a count of zero rather than
            # left out, because "no cell survived the gate" and "the species
            # was not run" are different statements.
            "n_agents_admitted": len(adm),
            "kappa_median_admitted": (statistics.median(c["kappa"]
                                                        for c in adm)
                                      if adm else None),
            "kappa_permuted_median_admitted": (
                statistics.median(c["kappa_permuted"] for c in adm)
                if adm else None)}
    ...
    return {
        ...
        # Two readings of one run, kept apart. "admitted" holds the cells the
        # estimator's own support gate returns an estimate for, and is the
        # layer any claim about a species is entitled to rest on. "analysed"
        # holds every cell that reached the estimator, gate or no gate, and is
        # a diagnostic: it is what the permuted-label control is measured on,
        # because a control has to run wherever the estimator ran.
        "admitted": _layer(admitted),
        "analysed": _layer(table),
        "agents_accounted": {
            r["organism"]: {"available": r["n_drugs_available"],
                            "recorded": len(r.get("agents") or {}),
                            "analysed": sum(1 for c in (r.get("agents") or
                                                        {}).values()
                                            if "kappa" in c)}
            for r in results if r.get("n_drugs_available")},
        "cells": len(table),
...
def write_outputs(out: Path, results: list[dict]) -> None:
    ...
    (out / "atlas_table.json").write_text(
        dumps(table, indent=1) + "\n", encoding="utf-8")
    summary = summarise(table, results)
    (out / "atlas_summary.json").write_text(
        dumps(summary, indent=2) + "\n", encoding="utf-8")
    adm, ana = summary["admitted"], summary["analysed"]
    def fmt(value, signed=False):
        return (f"{value:+.4f}" if signed else f"{value:.3f}") if value is not None else "not computed"

    print(f"\nadmitted by the support gate: {adm['cells']} cells across "
          f"{adm['species']} species, median kappa "
          f"{fmt(adm['kappa_median'])}, median permuted "
          f"{fmt(adm['kappa_permuted_median'], True)}")
    print(f"analysed in all (diagnostic): {ana['cells']} cells across "
          f"{ana['species']} species, median kappa "
          f"{fmt(ana['kappa_median'])}, median permuted "
          f"{fmt(ana['kappa_permuted_median'], True)}")
    print(f"written: {out}")
