...
WHAT THIS ANSWERS. The cross-species atlas reads
...
def load(raw: Path, organism: str) -> pd.DataFrame | None:
    ...
    if not ast_path.is_file() or not cl_path.is_file():
        return None
    ast = pd.read_csv(ast_path, sep="\t", dtype=str, on_bad_lines="error")
    clusters = pd.read_csv(cl_path, sep="\t", dtype=str, on_bad_lines="error")
