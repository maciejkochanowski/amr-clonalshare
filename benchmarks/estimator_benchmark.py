...
THE ESTIMATORS, and what each targets.
  ...
  the design-corrected definition, S_a^2 / (S_a^2 + sigma^2) with S_a^2 = B_w /
...
def cohort(rng, n_groups, size, share, binary=False, prevalence=0.25,
    ...
    # Three quantities, three targets. ``eta`` is
...
#: uncertainty of a fresh draw of lineages. Both scores are recorded, so the
#: reader can see the difference rather than take it on trust.
...
def design() -> list[dict]:
    ...
    # quinolone cell, where the exact realised interval
