...
The designs below cover three features of real MIC collections that the
designs of ``calibrate_null.py`` do not: dilution panels on which most
readings fall on an end well, a two-component residual distribution within
lineages, and a laboratory effect that is partly aligned with lineage. The
driver reuses the loop, the outputs and the receipts of ``calibrate_null.py``;
only the design grid and the data-generating function differ, and the designs
have their own root seed.
...
effect of the MIC model (``covariate``). Without the term, a pooled analysis
is centred on the between-lineage share of the mixed collection rather than on
the generating value; the other two analyses show what removes that. The last
three designs add a second covariate, the year of isolation, and adjust for
both.
"""
