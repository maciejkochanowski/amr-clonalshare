...
def summarize(directory,output):
    ...
    if data.duplicated(['phase','cell','replicate']).any(): raise ValueError('duplicate datasets')
    rows=[]
    for (phase,cell),group in data.groupby(['phase','cell']):
        row={k:group.iloc[0][k] for k in ['phase','cell','rho','groups','reading','size_pattern','residual','domain']}
        ...
        both=group.legacy_reportable&group.new_reportable
        diff=group.loc[both,'new_covered'].astype(float)-group.loc[both,'legacy_covered'].astype(float)
        row['paired_n']=int(both.sum()); row['paired_coverage_gain']=float(diff.mean()) if len(diff) else np.nan
        row['paired_gain_mcse']=float(diff.std(ddof=1)/np.sqrt(len(diff))) if len(diff)>1 else np.nan
        row['new_full_range_returns']=int((~group.new_reportable).sum())
        row['new_coverage_including_full_ranges']=float(group.new_covered.mean())
        row['chi_square_coverage']=float(group.loc[group.new_reportable,'chi_square_covered'].mean()) if row['reading']!='exact' else np.nan
        ...
        row['coverage_membership_mismatches']=int(((group.new_covered!=group.new_test_covered)&group.new_reportable).sum())
        rows.append(row)
    summary=pd.DataFrame(rows)
    output=Path(output); output.parent.mkdir(parents=True,exist_ok=True)
    summary.to_csv(output,index=False)
    counts=dict(chunks=len(paths),datasets=len(data),cells=int(data.cell.nunique()),
                phases=sorted(data.phase.unique().tolist()),sum_fit_seconds=float(data.seconds.sum()),
    ...
    print(summary[['cell','reading','size_pattern','rho','attempts','legacy_conditional_coverage','new_conditional_coverage','new_reportability','new_median_width']].to_string(index=False))
