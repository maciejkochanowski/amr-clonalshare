...
def generate(cell,replicate,phase):
    stage={'pilot':1,'confirmation':2}[phase]
    rng=np.random.default_rng(np.random.SeedSequence([ROOT_SEED,stage,cell['cell'],replicate]))
    ...
    inner_seed=int(np.random.SeedSequence([ROOT_SEED,stage,cell['cell'],replicate,991]).generate_state(1)[0])
...
def main():
    ...
    ap.add_argument('--phase',choices=['pilot','confirmation'],default='pilot')
    ...
    for rep in range(args.start,args.start+args.replicates):
        t=time.perf_counter(); a,b,c,edges,seed=generate(cell,rep,args.phase)
        row=dict(cell=args.cell,phase=args.phase,replicate=rep,rho=cell['rho'],n=len(c),groups=cell['groups'],
                 ...
                 legacy_low=np.nan,legacy_high=np.nan,legacy_reportable=False,legacy_covered=False,
                 legacy_estimate=np.nan,new_low=0.,new_high=1.,new_reportable=False,new_covered=True,
                 new_test_covered=True,ml_estimate=np.nan,chi_square_covered=False,lr_truth=np.nan,
        ...
        try:
            old=censored_clonal_share(a,b,c)
            row.update(legacy_low=old.ci_low,legacy_high=old.ci_high,legacy_reportable=old.estimable,
                       legacy_covered=bool(old.estimable and old.ci_low<=cell['rho']<=old.ci_high),
                       legacy_estimate=old.kappa)
        except Exception as exc:
            row['legacy_failure']=f'{type(exc).__name__}: {exc}'
        try:
            if cell['reading']=='exact':
                result=exact_gaussian_interval(a,c)
                row.update(new_low=result.low,new_high=result.high,new_reportable=True,
                           new_covered=result.low<=cell['rho']<=result.high,
                           new_test_covered=result.low<=cell['rho']<=result.high,
                ...
                row.update(lr_truth=lr,chi_square_covered=lr<=chi2.ppf(.95,1),new_test_covered=lr<=critical)
                if rep%args.bounds_every==0:
                    low,high,nfailed=profile_bounds(problem,fit,critical)
                    row.update(new_low=low,new_high=high,profile_failures=nfailed,
                               new_covered=low<=cell['rho']<=high,
                               new_reportable=bool(np.isfinite(critical) and nfailed==0))
                else:
                    row.update(new_low=np.nan,new_high=np.nan,new_covered=lr<=critical,
                               new_reportable=bool(np.isfinite(critical)))
    ...
    print(json.dumps({'cell':args.cell,'replicates':len(rows),'seconds':receipt['elapsed_seconds'],
                      'reportable':sum(r['new_reportable'] for r in rows),
                      'covered':sum(r['new_covered'] for r in rows)}))
