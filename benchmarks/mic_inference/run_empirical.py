...
old=censored_clonal_share(lo,hi,labels)
new=null_bootstrap_interval(lo,hi,labels,panel_edges=record['panel_edges'],panel=panel,covariate=covariate,n_boot=a.bootstrap,seed=record['seed'],workers=a.workers)
...
result=dict(observation=record,legacy=old.as_dict(),interval=new.as_dict(),
...
print(json.dumps(dict(agent=record['agent'],estimate=new.fit.rho,low=new.low,high=new.high,
    reportable=new.reportable,nulls=len(new.tests),seconds=result['elapsed_seconds'])))
