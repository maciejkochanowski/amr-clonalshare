...
def generate(cell, replicate, phase, *, root_seed=ROOT_SEED):
    stage = {'pilot': 1, 'confirmation': 2}[phase]
    rng = np.random.default_rng(np.random.SeedSequence(
        [root_seed, stage, cell['cell'], replicate]))
    ...
    seed = int(np.random.SeedSequence([root_seed, stage, cell['cell'], replicate, 991]).generate_state(1)[0])
...
def main():
    ...
    for rep in range(args.start, args.start+args.replicates):
        ...
        a, b, labels, edges, seed = generate(cell, rep, args.phase, root_seed=args.root_seed)
        row = dict(cell=cell['cell'], phase=args.phase, replicate=rep, rho=cell['rho'], root_seed=args.root_seed,
        ...
        row.update(generation_mean=cell.get('mean', 0.), generation_total_sd=cell.get('total_sd', 1.),
            ...
            legacy_low=np.nan, legacy_high=np.nan, legacy_reportable=False,
            legacy_covered=False, legacy_estimate=np.nan, new_low=0., new_high=1.,
            new_reportable=False, new_covered=True, new_test_covered=True,
        ...
        try:
            old = censored_clonal_share(a, b, labels)
            row.update(legacy_low=old.ci_low, legacy_high=old.ci_high,
                legacy_reportable=old.estimable, legacy_estimate=old.kappa,
        ...
        try:
            ...
            row.update(ml_estimate=test.unrestricted_fit.rho, lr_truth=test.statistic,
                critical=test.critical, pvalue=test.pvalue, new_reportable=test.reportable,
                new_covered=test.accepted, new_test_covered=test.accepted,
            ...
            if test.reportable:
                row.update(new_low=np.nan, new_high=np.nan)
            else:
            ...
            if args.bounds_every and rep % args.bounds_every == 0:
                ...
                row.update(new_low=interval.low, new_high=interval.high,
                    new_covered=covered, new_reportable=interval.reportable,
            ...
            row.update(new_reportable=False, new_low=0., new_high=1.,
                       new_covered=True, new_test_covered=True)
    ...
    print(json.dumps({'cell': args.cell, 'datasets': len(rows),
        ...
        'covered': sum(r['new_covered'] for r in rows), 'seconds': receipt['elapsed_seconds']}))
