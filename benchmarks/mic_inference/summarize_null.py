...
def main():
    ...
    for cell, group in data.groupby('cell'):
        ok = group.get('test_reportable', group.interval_reportable).astype(bool)
        accepted = group.get('pointwise_accepted', group.interval_test_covered).astype(bool)
        success = int((ok & accepted).sum())
        count = int(ok.sum())
        low, high = wilson(success, count)
        computed = group.bounds_computed.astype(bool)
        resolved = computed & group.interval_reportable.astype(bool)
        anti = group.anti_conservative_inversion_mismatch.astype(bool)
        rows = group.loc[resolved]
        details.append(dict(cell=cell,
            ...
            inversion_covered=int((resolved & group.interval_covered).sum()),
            inversion_median_width=float((rows.interval_high-rows.interval_low).median()) if len(rows) else np.nan,
            anti_conservative_inversion_mismatches=int(anti.sum()),
            mean_redrawn_simulations=float(group.bootstrap_redrawn.mean())))
    summary = summary.merge(pd.DataFrame(details), on='cell', validate='one_to_one')
