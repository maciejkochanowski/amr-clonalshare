...
WHY THIS EXISTS. Theory alone asserts that the e-value
...
under the Benjamini-Yekutieli arbitrary-dependence correction, which is the natural
alternative because cross-resistance makes the agents dependent with unknown
...
RULES = ("bh_accumulated", "by_accumulated", "lond_batch", "ebh_accumulated",
         "ebh_product", "lond_finite_horizon", "ebh_sequential")
...
def run_replicate(seed_seq: np.random.SeedSequence, *, n_agents: int,
                  ...
                  evalue_only: bool = False) -> Dict[str, object]:
    """Simulate one panel, apply every rule at every look, return declarations.

    Two p-values and three e-values are formed at each look. The accumulated
    ones use every isolate collected so far and feed the two step-up
    procedures and the fixed-look e-BH arm. The per-year ones use that year's
    isolates alone and feed the online comparator and the running product of
    per-year e-values. The sequential e-value fits the lineage probabilities
    on every earlier year and scores the new year against the null maximised
    on it, the construction of ``sequential_e_process``, whose running product
    is a test supermartingale over the looks.

    ``evalue_only`` skips the permutation p-values and the accumulated and
    per-year e-values, leaving only the sequential arm; the panels are drawn
    from the same seed stream either way, so a run in that mode scores the
    sequential rule on exactly the panels the full run scored the others on.
    """
    ...
    e_seq = np.ones((T, A))
    for a in range(A):
        seq = sequential_e_process(
            [Y[t * int(n_per_year):(t + 1) * int(n_per_year), a] for t in range(T)],
            [lineage[t * int(n_per_year):(t + 1) * int(n_per_year)] for t in range(T)])
        e_seq[:, a] = [np.exp(min(v, 700.0)) for v in seq.log_e]
    if evalue_only:
        p_acc[:] = np.nan
        p_bat[:] = np.nan
        e_acc[:] = np.nan
        ...
        return _apply_rules(p_acc, p_bat, e_acc, e_bat, alpha=alpha,
                            is_null=panel["is_null"],
                            prevalence_seen=float(Y.mean()), e_seq=e_seq)
    for t in range(T):
    ...
    return _apply_rules(p_acc, p_bat, e_acc, e_bat, alpha=alpha,
                        ...
                        float(Y.mean()), e_seq=e_seq)


def _apply_rules(p_acc, p_bat, e_acc, e_bat, *, alpha, is_null,
                 prevalence_seen, e_seq=None) -> Dict[str, object]:
    ...
    skipped = bool(np.isnan(p_acc).all())
    if not skipped:
        ...
        decl["lond_finite_horizon"] = lond_declare(p_bat, alpha,
                                                   horizon="finite")
        out = np.zeros((T, A), dtype=bool)
        for t in range(T):
    ...
    if e_seq is not None:
        out = np.zeros((T, A), dtype=bool)
        for t in range(T):
            out[t, e_bh(e_seq[t], alpha=alpha)["rejected"]] = True
        decl["ebh_sequential"] = out
    ...
    for name in RULES:
        if name not in decl:
            rec[name] = {m: float("nan") for m in METRICS}
            continue
        d = decl[name]
        ever = d.any(axis=0)
        n_ever = int(ever.sum())
...
def _worker(args):
    ...
    return run_replicate(ss, null_fraction=cell["null_fraction"],
                         ...
                         evalue_only=bool(common.get("evalue_only", False)),
                         **{k: common[k] for k in
...
def environment() -> Dict[str, object]:
    ...
    import scipy
    import amr_clonalshare as pkg
    return {"python": sys.version.split()[0], "numpy": np.__version__,
            "scipy": scipy.__version__,
            "amr_clonalshare": getattr(pkg, "__version__", "unknown"),
            "package_file": Path(pkg.__file__).name, "platform": platform.platform(),
            "node": platform.node()}
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--evalue-only", action="store_true",
                    help="score only the sequential e-BH rule, on the same "
                         "panels the full run draws")
    ...
    common = {"n_agents": a.n_agents, "n_years": a.n_years,
              ...
              "evalue_only": bool(a.evalue_only)}
