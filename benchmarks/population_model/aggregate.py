...
#: The general method is run on 500 datasets a cell (PROTOCOL.md).
GENERAL_REPLICATES = 500


def _expected(data, family):
    design = {c["cell"]: c for c in cells() if c["family"] == family}
    general = bool((data.method == "general").all())
...
def main(argv=None) -> int:
    ...
    if len(argv) != 3 or argv[0] not in ("calibrate", "cover", "benefit", "evidence"):
        print(__doc__, file=sys.stderr)
        return 2
    step, source, target = argv[0], Path(argv[1]), Path(argv[2])
    {"calibrate": calibrate, "cover": cover, "benefit": benefit, "evidence": evidence}[step](source, target)
    return 0
