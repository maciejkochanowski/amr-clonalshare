...
lineage-membership share of the same isolates. ``mixing`` records the
fixed-cut-off interval and the package's check of the Gaussian law of lineage
effects on the same dataset, so that coverage can be read by the check's
decision. One row per replicate; a failed fit is a row with its reason, never
a missing row.
"""
...
FIELDS = ("cell", "family", "replicate", "method", "rho", "prevalence", "status",
          ...
          "share_latent", "mixing_statistic", "mixing_p", "mixing_rejected", "failure", "seconds")
...
def _mixing(k, m, rho, critical, seed):
    from amr_clonalshare._population_mixing import mixing_check
    native = numerics.fit_profile(k, m, critical_value=critical, truth_rho=rho)
    row = dict(status=native["status"], rho_hat=native["rho_hat"], lr_at_truth=native["lr_at_truth"],
               ci_low=native["ci_low"], ci_high=native["ci_high"])
    check = mixing_check(k, m, native, seed=seed)
    if check is not None:
        row.update(mixing_statistic=check["statistic"], mixing_p=check["p_value"],
                   mixing_rejected=int(check["rejected"]))
    return row
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--method", choices=("lr", "general", "benefit", "mixing"), required=True)
    parser.add_argument("--critical-value", type=float)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args(argv)
    if args.method == "mixing" and args.critical_value is None:
        parser.error("--method mixing needs --critical-value")
    ...
    for replicate in range(args.start, args.start + args.replicates):
        ...
        try:
            if args.method == "lr":
                ...
                row.update(_mixing(k, m, cell["rho"], args.critical_value, seed))
            else:
