...
WHY THIS EXISTS. Two substitutes are the natural comparators. The first is
...
def anchor_reml(y, lineage) -> dict:
    ...
    answers is whether the shipped arm is the mixed model it is
    reported as.
    """
    y = np.asarray(y, dtype=float).ravel()
...
def run_anchor() -> dict:
    ...
    mixed model is not established, and the arm should then not be
    reported as the mixed model.
    """
    rows = []
