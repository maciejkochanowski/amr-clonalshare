...
truly zero, and power when it is not. Beside the percentile interval the
package reports, four other intervals are computed from the same bootstrap
draws (basic, bias-corrected, bias-shifted and studentised), and the coverage
of the percentile interval is also recorded by the dataset's shared support,
in bins of 0.05, which is what the support gate of the package is read from.
...
from amr_clonalshare.clonality import (
    DEFAULT_MIN_SHARED_SUPPORT,
    _bootstrap_components,
    _shared_index,
    decompose_prevalence_difference,
)

#: Intervals computed from the bootstrap draws of every dataset.
METHODS = ("percentile", "basic", "bias_corrected", "bias_shifted", "studentised")
#: Lower edges of the shared-support bins of the percentile coverage.
SUPPORT_BINS = np.round(np.arange(0.0, 1.0, 0.05), 2)
...
def intervals(estimate: float, draws: np.ndarray) -> dict:
    """Five 95 % intervals from one set of bootstrap draws of one component."""
    draws = draws[np.isfinite(draws)]
    q = np.percentile(draws, [2.5, 97.5])
    mean, sd = draws.mean(), draws.std(ddof=1)
    below = (np.sum(draws < estimate) + 0.5 * np.sum(draws == estimate)) / draws.size
    z0 = norm.ppf(np.clip(below, 1e-4, 1 - 1e-4))
    centre = 2 * estimate - mean
    return {"percentile": q,
            "basic": np.array([2 * estimate - q[1], 2 * estimate - q[0]]),
            "bias_corrected": np.percentile(draws, 100 * norm.cdf(2 * z0 + norm.ppf([0.025, 0.975]))),
            "bias_shifted": q - 2 * (mean - estimate),
            "studentised": np.array([centre - 1.959963984540054 * sd, centre + 1.959963984540054 * sd])}
...
def run_cell(cell: dict) -> dict:
    ...
    other = {k: {m: 0 for m in METHODS} for k in hits}
    other_widths = {k: {m: [] for m in METHODS} for k in hits}
    by_support = {k: np.zeros((len(SUPPORT_BINS), 2), dtype=int) for k in hits}
    ...
    for _ in range(cell["replicates"]):
        ...
        # The bootstrap draws of the package are regenerated from the same
        # generator state, so the other intervals use exactly those draws.
        ...
        twin.bit_generator.state = state
        codes_a, codes_b, lineages = _shared_index(lina, linb)
        boot = _bootstrap_components(codes_a, ya, codes_b, yb, len(lineages), cell["n_boot"], twin)
        support_bin = min(int(result["shared_support_isolate_share"] / 0.05 + 1e-9), len(SUPPORT_BINS) - 1)
        ...
        for key, true_value in (("composition", population["true_composition"]),
            ...
            by_support[key][support_bin] += (1, int(low <= true_value <= high))
            computed = intervals(result[key], boot[:, 0 if key == "composition" else 1])
            if not np.allclose(computed["percentile"], [low, high], rtol=0, atol=1e-12):
                raise AssertionError("regenerated bootstrap draws differ from the package's")
            for method, (m_low, m_high) in computed.items():
                other[key][method] += int(m_low <= true_value <= m_high)
                other_widths[key][method].append(m_high - m_low)
    ...
    for key in ("composition", "within_lineage"):
        ...
        for method in METHODS:
            record[f"{key}_{method}_coverage"] = float(other[key][method] / n) if n else None
            record[f"{key}_{method}_mean_width"] = (float(np.mean(other_widths[key][method]))
                                                    if n else None)
        record[f"{key}_coverage_by_support"] = [
            {"support_from": float(edge), "n": int(count), "covered": int(covered)}
            for edge, (count, covered) in zip(SUPPORT_BINS, by_support[key]) if count]
    return record
...
def main() -> int:
    ...
    parser.add_argument("--min-shared-support", type=float, default=DEFAULT_MIN_SHARED_SUPPORT)
