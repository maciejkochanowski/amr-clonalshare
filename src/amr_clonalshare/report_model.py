...
def build_report(record: dict, summary: dict, *,
    ...
    v3_profiles = {k: r for k, r in profiles.items() if k not in general_profiles}
    ...
    if rows:
        ...
        s.table("Table 1b", "Lineage structure behind each share: lineages with "
                "at least two isolates, the effective number of lineages (inverse "
                "of the sum of squared lineage shares) and the share of isolates in "
                "lineages with at least two members (support). A high support with "
                "few effective lineages means the share rests on a few lineages.",
                [("Trait", "lab"), ("Lineages", "num"), ("Repeated", "num"),
                 ("Effective", "num"), ("Support", "num")],
                [(r["name"], num((md.get("clonal_share") or {}).get(r["name"], {}).get("n_groups"), 0),
                  num(r.get("repeated"), 0), num(r.get("effective"), 1), pct(r.get("support")))
                 for r in rows])
    if point_only:
    ...
    if feasibility:
        ...
        s.table("Table 2a", "Per-agent input feasibility and support scenario.",
    ...
    if cz_agents:
        ...
        if drows:
            s.figure(fig_label(), "dilution", drows[:18],
                     ...
                     "interval for the Gaussian population model (thin) and the realised "
    ...
    if general_profiles:
        ...
        accepted_general = all(r.get("validation_status") == "confirmation_accepted" and
                               ((r.get("provenance") or {}).get("protocol") or {}).get("accepted") is True
                               for r in general_profiles.values())
        s.p("The finite-panel simulation confirmed the coverage target of this method."
            if accepted_general else "The coverage of this method is not confirmed for one or more result records.")
        ...
        def general_reading(r):
            ...
            return ("Computed; empirical confirmation accepted"
                    if r.get("validation_status") == "confirmation_accepted" and
                    ((r.get("provenance") or {}).get("protocol") or {}).get("accepted") is True
                    else "Computed; confirmation pending")
    ...
    if v3_profiles:
        ...
        first = next(iter(v3_profiles.values()))
        ...
        s.table("Table P1", "Population-model profile sets; finite-grid calibration "
                ...
                [("Agent", "lab"), ("Population liability ICC", "num"), ("Model profile interval", "num"),
                 ("Fitted prevalence", "num"), ("Status", "lab")],
                [(name, num(r.get("rho_hat")), interval(r.get("ci_low"), r.get("ci_high")),
                  num(r.get("prevalence_hat")), r.get("status", "not recorded"))
                 for name, r in v3_profiles.items()])
        ...
        s.p("Constant outcomes or too few repeated groups retain an explicitly "
            ...
            "from the classical 0.90 support gate. Counts, exclusions, numerical "
    ...
    if not profile_only:
        interval_terms.append(
            ...
            "historical finite-grid coverage is not a universal guarantee.")
    if v3_profiles:
        interval_terms.append(
    ...
    s.callout("Terms used in this report", [
        ...
        "**Classical support policy.** The share of isolates in repeated "
        "lineages; below %s the classical share is not admitted for interpretation; diagnostic values may remain in the record. This is separate "
        "from the optional population model's repeated-group policy."
        % pct(gate, 0),
