...
-------------------------
Two different questions share one point estimate and must not share one
interval.
...
What would mean this measured the wrong thing
---------------------------------------------
Each interval must be scored against its own truth. Measured at thirty lineages
of twenty isolates and a true share of 0.30 over 3000 replicates: the
variance-component interval covers its own target 0.951 of the time and the
realised target 1.000; the realised interval covers its own target 0.950 and
the superpopulation target 0.658. Had the second row not fallen away from
nominal, the two estimands would not be distinguishable and this module would
have no reason to exist.

What was tried and discarded
----------------------------
Recorded here rather than deleted. A Box-type deflation of the degrees of
freedom by the estimated residual kurtosis was meant to repair heavy tails; it
made coverage worse, not better, at every tail weight tested (0.872 to 0.040 at
t(4), 0.941 to 0.487 at t(10)), and is not used. A within-lineage percentile
bootstrap was meant to provide an assumption-free fallback; it covered 0.786
under the very process the method is derived against, so it is not a fallback
either. What remains is the exact interval together with a gate on the residual
kurtosis, whose limit is read off the measured coverage curve.

What the gate is and is not
---------------------------
Measured conditional on its own decision (90 cells, 10000 replicates each). Given an open gate the
interval covers 0.947 to 0.957 under the Gaussian law and 0.929 to 0.961 on
binary traits generated at baseline probability 0.25 in a logit-normal
model (marginal prevalence varies with the effect variance). On a
...
sample looked Gaussian, not that the interval holds. The gate reads fitted,
leverage-scaled within-lineage residuals, so the marginal Bernoulli kurtosis
does not define a universal prevalence band for acceptance. In the logit-normal
arm with baseline probability 0.085, the gate often refused, and coverage of
the withheld intervals ranged from 0.86 to 0.96. Binary intervals are an
approximation even when this empirical gate opens.
"""
...
#: interval is refused by an empirical screening rule. The
...
#: ``benchmarks/realised_calibration.py``, and is read off it. Below it the
#: unconditional measured coverage on that calibration curve runs 0.93 to 0.95;
...
def _label_strings(labels: np.ndarray) -> np.ndarray:
    ...
    collapse to one ``__missing__`` level here, exactly as
    :func:`attribution._codes` documents it, rather than becoming several
    levels or an error; :func:`realised_share` then sets those isolates aside,
    because an untyped isolate is not a member of any lineage.
    """
    from .attribution import _is_untyped
    out = np.empty(labels.shape, dtype=object)
    for i, v in enumerate(labels.ravel()):
        out.ravel()[i] = _MISSING if _is_untyped(v) else str(v)
...
class RealisedShare:
    ...
    estimable: bool
    reason: str
    #: Isolates set aside because they carry no lineage label. An untyped
    #: isolate is not a member of any lineage, and a level made of every
    #: isolate whose typing failed measures the typing process rather than
    #: lineage membership, so those isolates are dropped and the estimate
    #: describes the typed ones. Counted on the cohort as supplied, so an
    #: isolate that is both untyped and non-finite is counted here and in
    #: ``n_dropped_non_finite``.
    n_dropped_untyped: int = 0
    #: Isolates set aside because the trait value was not finite. A missing
    #: well is normal in a susceptibility panel, and a non-finite value
    #: belongs to no lineage mean, so those isolates are dropped and the
    #: count is carried because the estimate then describes a subset of the
    #: cohort. Counted on the cohort as supplied, as ``n_dropped_untyped`` is.
...
def realised_share(y: Sequence[float], lineage: Sequence,
    ...
    strings = _label_strings(labels)
    typed = strings != _MISSING
    finite = np.isfinite(values)
    # Both counts are taken on the cohort as supplied. Counting the untyped
    # after the finite filter under-reported them, because an isolate that is
    # both untyped and unread was never offered to the typed mask.
    n_dropped_untyped = int((~typed).sum())
    n_dropped_non_finite = int((~finite).sum())
    keep = typed & finite
    values, strings = values[keep], strings[keep]
    if values.size == 0:
        return RealisedShare(float("nan"), float("nan"), float("nan"),
                             float("nan"), float("nan"), 0, 0, float("nan"),
                             float("nan"), float("nan"), False,
                             "no finite observations with a lineage label",
                             n_dropped_untyped, n_dropped_non_finite)
    _, code = np.unique(strings, return_inverse=True)
    ...
    if not (np.isfinite(low) and np.isfinite(high)):
        ...
        # Both endpoints solve to a noncentrality of zero: the observed ratio
        # of mean squares falls below the alpha/2 point of the central F, so
        # the set of noncentralities the observation does not reject is empty
        # and the pair (0, 0) the inversion returns is not an interval.
        estimable = False
        reason = (f"the observed mean-square ratio {f_ratio:.3f} falls below "
                  f"the {alpha / 2.0:.3f} point of the central F on "
                  f"{df_between:.0f} and {df_within:.0f} degrees of freedom, "
                  f"so the inverted confidence set for the noncentrality is "
                  f"empty and the interval closes on zero")
