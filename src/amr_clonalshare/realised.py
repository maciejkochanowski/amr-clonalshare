...
---------------------------------------------
Each interval must be scored against its own truth, and
``benchmarks/realised_calibration.py`` scores each against both: the
variance-component interval covers its own target and the realised target,
and the realised interval covers its own target but not the superpopulation
target. Were that cross-scoring not to break coverage, the two estimands
would not be distinguishable and this module would have no reason to exist.

Heavy tails
-----------
Two repairs for heavy-tailed residuals, a Box-type deflation of the degrees
of freedom by the estimated residual kurtosis and a within-lineage percentile
bootstrap, are kept as cells of that calibration because neither restores
coverage; neither is used. What remains is the exact interval together with a
gate on the residual kurtosis, whose limit is read off the same calibration.

What the gate is and is not
---------------------------
On a heavy-tailed continuous process the gate refuses most cohorts, and the
few it admits can cover far below the nominal level: a sample-kurtosis gate
selects the light-tailed draws of a heavy-tailed process. The gate is
therefore a refusal rule, not a certificate, and an open gate on a continuous
trait says the sample looked Gaussian, not that the interval holds. The gate
reads fitted, leverage-scaled within-lineage residuals; for a binary call
their excess kurtosis is governed by the prevalence, so on binary traits the
gate acts as a prevalence window. Binary intervals are an approximation even
when the gate opens.
"""
...
#: interval is refused by an empirical screening rule. The limit is read off
#: the coverage curve of ``benchmarks/realised_calibration.py``: the largest
#: median excess kurtosis at which coverage of the realised share stayed at or
#: above 0.93, rounded to two decimals.
KURTOSIS_LIMIT = 0.99
