...
The idea that organises this module
-----------------------------------
A dichotomised non-wild-type call, a recorded MIC and a censored reading are
not three analyses. They are one likelihood at three interval widths. A
wild-type call is the interval (-inf, log2 cut-off]; a non-wild-type call is
(log2 cut-off, +inf); a recorded MIC of 8 mg/L on a doubling panel is (log2 4,
log2 8]; a reading on the lowest tested well is left-censored and one on the
highest is right-censored. The estimator below takes intervals and nothing
else, so the input mode changes only how much information an observation
carries, never what is estimated. Two tests in ``tests/test_censored.py``
enforce that: with degenerate intervals the result must equal the continuous
estimator, and with single-cut intervals it must equal the binary one.

What dichotomisation costs
--------------------------
That cost is therefore measurable rather than asserted. Cohen
...
The assumption that must be declared
------------------------------------
Cohorts frequently record no censoring operator, and yet pile mass on the end
wells of a panel. Treating those readings as censored is an assumption about
the coarsening mechanism, not an observation. It is the coarsened-at-random
condition of Heitjan and Rubin (1991), under which the coarsening is ignorable
for likelihood inference. ``panel_geometry`` reports the end-well mass so the
assumption is visible, and ``sensitivity_endpoints`` recomputes the share with
the end wells treated as exact readings instead, which brackets the effect of
getting it wrong.

Why the group means are shrunk
------------------------------
A lineage whose isolates all sit on the top well carries no information about
how far above it they lie; its latent mean is not identified and an unshrunk
maximum-likelihood estimate runs away. The empirical-Bayes factor pulls such a
lineage back to the cohort mean in proportion to how little it says, which is
what makes heavy censoring survivable rather than fatal. Zhang, Wang and
O'Connor (2021) declined to estimate variance components under large censored
fractions; the answer here is to shrink, to report the panel geometry, and to
refuse when the geometry says the question cannot be answered.
"""
...
from .attribution import _codes, _is_untyped, _rng
...
class CensoredShare:
    ...
    #: interval: on the first run of the design grid, with 200 draws a cohort,
    #: it covered a median 0.61 against a nominal 0.95, because resampling
    #: lineages does not reproduce the sampling distribution of a component
    #: built from those lineages. The shipped grid was run with the bootstrap
    #: off.
    boot_low: float
    boot_high: float
...
def _lattice(v: np.ndarray, wells=None):
    ...
    With ``wells`` given, every reading is placed on the nearest tested well.
    ...
    if wells is not None:
        w = np.asarray([x for x in np.asarray(wells, dtype=float).ravel()
                        if np.isfinite(x) and x > 0])
        if w.size == 0:
            raise ValueError("wells holds no positive finite concentration")
        lattice = np.unique(np.log2(w))
        idx = np.abs(lv[:, None] - lattice[None, :]).argmin(axis=1)
...
def _trunc_moments(a, b, m, s):
    ...
    al = (a - m) / s
    be = (b - m) / s
    # log(Phi(be) - Phi(al)) computed stably. An interval in the upper tail
    # is reflected, Phi(be) - Phi(al) = Phi(-al) - Phi(-be), because log_ndtr
    # is accurate for large negative arguments and saturates at zero for large
    # positive ones, where the difference of two logarithms both within
    # rounding of zero carries no information. Without the reflection an
    # interval more than about seven residual standard deviations above the
    # mean returned the mean itself as its conditional expectation.
    upper = al > 0.0
    lo_z = np.where(upper, -be, al)
    hi_z = np.where(upper, -al, be)
    la = log_ndtr(lo_z)
    lb = log_ndtr(hi_z)
    with np.errstate(divide="ignore", invalid="ignore"):
        logZ = lb + np.log1p(-np.exp(np.minimum(la - lb, -1e-300)))
...
def censored_clonal_share(lo, hi, lineage, *, n_boot: int = 200,
    ...
    This is not the same quantity as ``attribution.clonal_share``, and the
    ...
    Three widths are returned and only one of them is an interval, as
    ``docs/methodology.md`` section 3.4 sets out. ``ci_low`` to ``ci_high``
    inverts the variance ratio the two mean squares form and is the one to
    report. ``boot_low`` to ``boot_high`` is the cluster bootstrap, which
    draws whole lineages and is kept as an assumption-free check rather than
    as the interval. ``profile_low`` to ``profile_high`` is the
    likelihood-ratio width, which measures how sharply this cohort pins the
    share down and is not a confidence statement at all.
    """
    ...
    keep = typed & ~(np.isnan(lo) | np.isnan(hi))
...
def profile_interval(lo, hi, lineage, *, alpha: float = 0.05, grid: int = 41):
    ...
    This is not a confidence interval, and the calibration says so rather than
    ...
    means. The interval to report is the variance-ratio inversion in
    ``CensoredShare.ci_low`` and ``ci_high``; the bias-corrected cluster
    bootstrap beside it, in ``boot_low`` and ``boot_high``, is a check and
    not the interval either.
    """
    lo = np.asarray(lo, dtype=float).ravel()
    hi = np.asarray(hi, dtype=float).ravel()
    labels = np.asarray(lineage, dtype=object).ravel()
    keep = (np.array([not _is_untyped(v) for v in labels], dtype=bool)
            & ~(np.isnan(lo) | np.isnan(hi)))
    lo, hi = lo[keep], hi[keep]
    code = _codes(labels[keep])
...
def _variance_ratio_interval(lo, hi, code, G, means, sigma, tau2,
    ...
    Measured over the design grid, coverage for exact and dilution readings
    is 0.93 to 0.98 by true share and 0.85 to 1.00 cell by cell over 40 cells
    of 200 simulated cohorts, across shapes from 15 lineages of 50 to 100 of
    6 and lowest at a hundred lineages of six isolates. The cluster bootstrap
    covered a median 0.61 on the first run of that grid, with 200 draws a
    cohort; the shipped grid was run with the bootstrap off.
    """
    parts = _ratio_and_degrees(lo, hi, code, G, means, sigma, tau2)
    if parts is None:
        return float("nan"), float("nan")
