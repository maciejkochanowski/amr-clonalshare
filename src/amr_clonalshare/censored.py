...
The gates rest on a finite simulation grid and apply to its scenarios only.
"""
...
# wearing a number, and the interval mode is required. A reading convention,
# not a calibrated limit.
END_WELL_LIMIT = 0.05
...
# in that position. The limit is read off the sweep of
# ``benchmarks/censored_gates.py`` (``beyond_panel``), the bias of the estimate
# against the share of isolates in such lineages: the largest multiple of 0.1
# at which the absolute bias stays under 0.05.
CENSORED_GROUP_LIMIT = 0.5
# Non-wild-type prevalence outside which a single cut point no longer carries
# the latent share, read off the sweep of ``benchmarks/censored_gates.py``
# (``single_cut``): the window, symmetric about one half, in which the absolute
# bias of the estimate stays under 0.05 at every true share. In the tails the
# estimate drifts above a true share and its interval stops covering it. Inside the window the estimate is reported,
# but its interval is a reading of the call rather than a calibrated interval,
# and the record says so in ``notes``. A dilution at the same prevalence is
...
class CensoredShare:
    ...
    estimable: bool
    reason: str
    #: The share carried by the lineages this cohort actually holds. The
    #: noncentral F of :mod:`amr_clonalshare.realised` is inverted at the
    #: mean-square ratio the fit implies, not an observed one, with residual
    #: degrees of freedom scaled by the information the censoring leaves, so
    #: this is an approximation and not the exact pivot of that module, and no
    #: kurtosis gate is applied. ``realised_status`` says so in the record.
    ...
    realised_status: str = "plug_in_mean_square_ratio; approximate_residual_df; not_the_exact_pivot"
...
def intervals_from_mic(values, *, operators=None, treat_end_wells_as_censored=True,
    ...
    less-than-or-equal and greater-than-or-equal signs.
    """
...
def _fit_moment_iteration(lo, hi, code, G, *, iters=1000, tol=1e-10, fixed_scale=None, diagnostics=None):
    """Approximate empirical-Bayes moment iteration for the interval model.

    These updates are not an exact EM or REML maximisation of the marginal
    likelihood.
...
def censored_clonal_share(lo, hi, lineage) -> CensoredShare:
    ...
    m, s, tau2, _g = _fit_moment_iteration(lo, hi, code, G, fixed_scale=fixed, diagnostics=fit_diagnostics)
    denom = tau2 + s * s
    ...
    if over_limit:
        ...
        notes.append(
            ...
            f"{fully_share:.0%} of isolates, below the reporting threshold of "
            f"{CENSORED_GROUP_LIMIT:.0%}")
    if not identified:
        notes.append(
...
def _realised_interval_from_fit(lo, hi, code, G, means, sigma, tau2,
    ...
    The noncentral F is inverted at the ratio the fit implies, with residual
    degrees of freedom scaled by the information the censoring leaves: an
    approximation, not the exact pivot of uncensored data. See :mod:`amr_clonalshare.realised` for the estimand and for
