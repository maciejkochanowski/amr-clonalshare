...
class DatasetConfig:
    ...
    def validate(self) -> None:
        ...
        if self.phenotype_kind not in ("legacy", "clinical_sir", "wt_nwt", "binary"):
            raise ConfigError("dataset.phenotype_kind must be legacy, clinical_sir, wt_nwt or binary")
        if self.phenotype_kind in ("wt_nwt", "binary") and self.phenotype_intermediate is not None:
            raise ConfigError("dataset.phenotype_intermediate applies only to clinical_sir or legacy")
        if self.duplicate_policy not in ("error", "drop_conflicts", "legacy"):
            raise ConfigError("dataset.duplicate_policy must be error, drop_conflicts or legacy")
...
class PopulationProbitConfig:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.interval_method, str) or self.interval_method not in ("fixed_v3", "general"):
            raise ConfigError("population_probit.interval_method must be fixed_v3 or general")
...
def from_dict(raw: Dict[str, Any], config_path: Optional[Path] = None) -> Config:
    ...
    dataset = DatasetConfig(
        ...
        phenotype_kind=ds.get("phenotype_kind", "legacy"),
    ...
    population_probit = PopulationProbitConfig(
        ...
        interval_method=profile_raw.get("interval_method", "fixed_v3"),
        ...
        cache_dir=profile_raw.get("cache_dir"))
