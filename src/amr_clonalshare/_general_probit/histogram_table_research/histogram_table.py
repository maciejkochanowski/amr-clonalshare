"""Research-only histogram table evaluation; no confidence interval or validity claim.

A finite grid defines a different statistic from adaptive continuous profiling.
The same frozen quadrature supplies table entries. Impossible bins are masked
...
FROZEN_NUMERICAL_SHA256='dededb84f429ba84602aef048d2642be725981dc5982719436b68d28d3927668'
...
class HistogramTable:
    ...
    def identity(self):
        return dict(geometry=list(self.geometry),theta=[[float(a).hex(),float(r).hex()] for a,r in self.theta],
            ...
            numerical_source_sha256=self.numerical_source_sha256,normalization='unchanged_frozen_quadrature',
...
def build_table(sizes,theta,*,memory_budget_bytes=256*2**20,count_block=128):
    _,m=frozen.validate_counts(np.zeros(len(sizes)),sizes)
    ...
    source=hashlib.sha256(Path(frozen.__file__).read_bytes()).hexdigest()
    if source!=FROZEN_NUMERICAL_SHA256:raise RuntimeError('Frozen numerical source changed')
    offsets={};bins=0
    ...
    for size,offset in offsets.items():
        for j,(a,rho) in enumerate(points):
            for begin in range(0,size+1,count_block):
                ...
                logp[offset+begin:offset+end,j]=frozen.selected_count_log_probabilities(
...
def grid_lr_scores(loglikelihoods,theta,null_rho):
    """Experimental finite-grid LR score only; this is not a confidence set.
