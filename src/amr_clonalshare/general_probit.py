...
def _protocol() -> dict:
    ...
    for path, expected in record["sources"].items():
        if hashlib.sha256(root.joinpath(*path.replace("\\", "/").split("/")).read_bytes()).hexdigest() != expected:
            raise RuntimeError("Selected general method source identity changed")
    if hashlib.sha256(Path(numerical.__file__).read_bytes()).hexdigest() != record["numerical_source_sha256"]:
        raise RuntimeError("Selected numerical source identity changed")
...
class GeneralProbitResult:
    ...
    validation_status: str = "confirmation_pending"
    rho_hat: None = None
...
def general_probit_icc(counts, sizes, *, seed: int = 42, case_key: str = "query",
    ...
    """Compute the selected method for the complete retained input geometry.
    ...
    Otherwise it reports the selected bootstrap hull, with components retained
    as diagnostics. Fixed method settings cannot be tuned through this API.
    Finite-panel empirical confirmation is independently accepted; this remains
    an approximate method with fitted-nuisance and continuum coverage.
    """
