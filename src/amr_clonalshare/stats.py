...
def effective_dimension(X: np.ndarray) -> float:
    ...
    if A.ndim != 2 or A.shape[1] == 0:
        return 0.0
    # A missing result in one agent drops that isolate, not the agent: the
    # correlation is taken over the isolates every kept agent has read.
    A = A[np.isfinite(A).all(axis=1)]
    keep = A.std(axis=0) > 0 if len(A) else np.zeros(A.shape[1], dtype=bool)
    if keep.sum() == 0:
        return float("nan")
...
def benjamini_hochberg(pvals, q: float = 0.05, *,
    ...
    if dependence not in ("independent", "arbitrary"):
        raise ValueError("dependence must be independent or arbitrary")
    pvals = np.asarray(pvals, dtype=float)
    if pvals.ndim != 1:
        raise ValueError("p-values must be a one-dimensional vector")
    finite = np.isfinite(pvals)
    if np.any(finite & ((pvals < 0.0) | (pvals > 1.0))):
...
def permutation_pvalue(null_stats, observed: float, *,
    ...
    legitimately be 0. Adding one to numerator and denominator gives a valid
    Monte Carlo test under exchangeability; it is not an unbiased tail estimator. Phipson & Smyth (2010), "Permutation p-values should never
    be zero", Stat Appl Genet Mol Biol 9(1):39, doi:10.2202/1544-6115.1585.
    """
    s = np.asarray(list(null_stats), dtype=float)
    if s.ndim != 1 or not np.isfinite(s).all() or not np.isfinite(observed):
        raise ValueError("permutation statistics and observed statistic must be finite")
    if tail not in ("greater", "less"):
        raise ValueError("tail must be greater or less")
    B = s.size
    if B == 0:
        # the formula's own value: no exceedance can be counted, so p = 1
        return 1.0
    b = int((s >= observed).sum()) if tail == "greater" else int((s <= observed).sum())
