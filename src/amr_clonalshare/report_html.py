...
share read from the recorded dilution beside the share of the call, and the
readings per lineage and dilution interval as a heat map. A figure is drawn
only where the record holds what it needs.
...
from .report_model import HEATMAP_LINEAGES, Block, build_report

__all__ = ["render_html_report"]

INK, MID, FAINT = "#000000", "#3D3D3D", "#6E6E6E"
RULE, PAPER = "#BFBFBF", "#FFFFFF"
ACCENT, FLAG = "#0B2545", "#A63D40"
...
def _fig_dilution(rows) -> str:
    ...
    for i, r in enumerate(rows):
        ...
        if _finite(r.get("lo")) and _finite(r.get("hi")):
            p.append('<line x1="%s" y1="%s" x2="%s" y2="%s" stroke="%s" '
                     'stroke-width="0.7" opacity="0.8"%s/>'
                     % (_r(X(r["lo"])), _r(y), _r(X(r["hi"])), _r(y), col,
                        '' if r.get("calibrated") else ' stroke-dasharray="2.4 1.6"'))
...
def _edge(v) -> str:
    """A log2 panel edge as the concentration it stands for."""
    if v is None:
        return "∞"
    c = 2.0 ** float(v)
    return ("%g" % c) if c >= 1 else ("%.2g" % c)


def _fig_heatmap(panels) -> str:
    """One heat map per agent: lineages in rows, dilution intervals in
    columns, cell darkness proportional to the count within the agent."""
    if not panels:
        return ""
    W, L, R, T, GAP, CELL = 470, 0, 46, 32, 30, 15.0
    blocks = []
    for name, d in panels:
        counts = d.get("counts") or []
        lineages = [str(x) for x in d.get("lineages") or []]
        ivs = d.get("intervals") or []
        if not counts or not lineages or not ivs:
            continue
        order = sorted(range(len(lineages)), key=lambda i: -sum(counts[i]))[:HEATMAP_LINEAGES]
        blocks.append((name, [lineages[i] for i in order], [counts[i] for i in order], ivs))
    if not blocks:
        return ""
    L = _left([lab for _, labs, _, _ in blocks for lab in labs])
    heights = [T + CELL * len(labs) + 26 for _, labs, _, _ in blocks]
    H = sum(heights) + GAP * (len(blocks) - 1)
    p = _open(W, H, "Readings per lineage and dilution interval")
    y0 = 0.0
    for (name, labs, counts, ivs), h in zip(blocks, heights):
        n_col = len(ivs)
        cw = (W - L - R) / max(n_col, 1)
        peak = max((c for row in counts for c in row), default=0) or 1
        p.append('<text x="%s" y="%s" class="ax" text-anchor="start">%s</text>'
                 % (_r(L), _r(y0 + 11), _esc(_short(name, 60))))
        for j, (a, b) in enumerate(ivs):
            x = L + j * cw
            p.append('<text x="%s" y="%s" text-anchor="middle" class="tk">%s</text>'
                     % (_r(x + cw / 2), _r(y0 + T - 6),
                        "≤%s" % _edge(b) if a is None else
                        (">%s" % _edge(a) if b is None else _edge(b))))
        for i, (lab, row) in enumerate(zip(labs, counts)):
            y = y0 + T + CELL * i
            p.append('<text x="%s" y="%s" text-anchor="end" class="rw">%s</text>'
                     % (_r(L - 8), _r(y + CELL / 2 + 3), _esc(_short(lab))))
            for j, c in enumerate(row):
                x = L + j * cw
                shade = 0.08 + 0.92 * (float(c) / peak) if c else 0.0
                p.append('<rect x="%s" y="%s" width="%s" height="%s" fill="%s" '
                         'fill-opacity="%.2f" stroke="%s" stroke-width="0.4"/>'
                         % (_r(x + 1), _r(y + 1), _r(cw - 2), _r(CELL - 2),
                            ACCENT if c else PAPER, shade if c else 1.0, RULE))
                if c:
                    p.append('<text x="%s" y="%s" text-anchor="middle" class="tk" '
                             'fill="%s">%d</text>'
                             % (_r(x + cw / 2), _r(y + CELL / 2 + 3),
                                PAPER if shade > 0.55 else INK, int(c)))
            p.append('<text x="%s" y="%s" class="rv">%d</text>'
                     % (_r(W - R + 6), _r(y + CELL / 2 + 3), int(sum(row))))
        p.append('<text x="%s" y="%s" text-anchor="middle" class="ax">MIC well '
                 '(upper edge, mg/L or the recorded unit; open at the panel ends)</text>'
                 % (_r((L + W - R) / 2), _r(y0 + h - 6)))
        y0 += h + GAP
    return "".join(p) + "</svg>"
...
_CSS = """
:root{--ink:#000000;--mid:#3D3D3D;--faint:#6E6E6E;--rule:#BFBFBF;
--paper:#FFFFFF;--accent:#0B2545;--flag:#A63D40}
...
def render_html_report(record: dict, summary: dict, *,
    ...
    add('<p class="sub">Run report. Every value is read from the record '
        "written by this run, or from the release's validation grid where "
        "the text says so; none is recomputed here.</p>")
    add('<div class="ident">')
