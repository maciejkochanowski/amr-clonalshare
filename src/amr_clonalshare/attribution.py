...
#: before a clonal share is flagged estimable. Read off the coverage curve
#: that ``benchmarks/attribution_calibration.py`` writes (cell D, coverage and
#: bias by support bin, 100 cohorts a bin), by the rule the file records: the
#: centre of the lowest bin in which the 95% interval covered the truth at
#: least 90% of the time. The curve is built on the worst case, every
#: lineage outside the large ones being a singleton, so a real cohort with a
#: graded size distribution clears it more easily than this threshold
#: implies. See ``SUPPORT_THRESHOLD_EVIDENCE``.
...
def layer_clonal_share(X, lineage, *, folds: int = 5, repeats: int = 20,
    ...
    structure: typing.Dict[str, typing.Any] = dict(
        ...
        n_positive_repeated=(float(X[sizes[code] >= 2, 0].sum())
