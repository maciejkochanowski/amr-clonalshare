...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--threads", type=int, default=None,
                   help="limit BLAS/OpenMP threads (sets OMP_NUM_THREADS et al.) and, "
                        "unless censored.workers is set, the worker processes")
...
def main(argv=None) -> int:
    ...
    if args.threads and not cfg.censored.workers:
        # --threads bounds the whole run: the default of one worker process
        # per CPU would otherwise multiply it.
        from dataclasses import replace
        cfg = replace(cfg, censored=replace(cfg.censored, workers=args.threads))
