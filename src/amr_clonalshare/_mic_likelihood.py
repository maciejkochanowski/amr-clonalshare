...
#: Upper end of rho in the maximisation box. Beyond it the residual standard
#: deviation falls below one hundredth of the total, the lineage integrals
#: become step functions of the effect, and no quadrature resolves them; the
#: limiting maximum at rho = 1 is handled separately (``_mic_boundary``).
RHO_MAX = 1-1e-4
...
class GaussianMICLikelihood:
    ...
    ``identified_levels`` refuses a covariate level whose readings are all
    censored on one side, since the data cannot identify its effect. The
    bootstrap turns it off: its statistic is the likelihood ratio maximised
    over the bounded nuisance box, which exists for such a dataset too, so a
    simulated dataset is not lost to a refusal meant for the observed one.
    """
    def __init__(self, lo, hi, lineage, covariate=None, *, identified_levels=True):
        ...
        for column, n in enumerate(self.n_levels if identified_levels else ()):
            for k in range(n):
    ...
    def evaluate(self, mean, total_sd, rho, *, order=24, coefficients=(), beta_gradient=True):
        ...
        if order <= 0:
            ...
            ll,dm,ds,dt,error,lineage_bin=integrate_lineages(shifted,counts,mode,scale,mean,sd,tau2,
                                                             strict=order<0)
            dr=.5*(dt/rho-ds/(1-rho))
            gradient=[dm+singleton_dm,ds+dt+singleton_ds,dr]
            if self.n_coefficients and lineage_bin is not None:
                gradient.extend(self._by_level(lineage_bin+singleton_bin))
            elif self.n_coefficients and beta_gradient:
    ...
    def objective(self,x,*,order=24):
        ...
        try:
            ...
            if self._resolved is None:
                raise
            # A trial point of a line search that no integrator resolves (a
            # corner of the box where the scale collapses) is scored as far
            # worse than the last resolved point, so the search steps back.
            return (self._resolved[0]+1e6,self._resolved[1])
    ...
    def fit(self,*,rho=None,start=None,order=24,verify=True,multistart=True,box_maximum=False):
        ...
        upper=[self.center+20*self.unit,np.log(self.unit)+6,RHO_MAX]+[20*self.unit]*p
        ...
        if verify:
            ...
            if error>1e-6 and order>0:
                # Gauss-Hermite disagrees with itself at the optimum: the lineage
                # integrals are too sharp for it (rho near one, or bins narrow
                # against the residual scale). Refit from here with the checked
                # piecewise rule, which resolves them directly, rather than
                # doubling the Gauss-Hermite order through several refits.
        ...
        if not fixed and theta[2]>RHO_MAX-1e-5:
            edge=True
