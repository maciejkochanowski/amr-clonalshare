...
from .attribution import MIN_REPEATED_LINEAGES, _codes
...
#: in lineages of at least this size; the estimator scores those isolates and
#: sets the singletons aside.
MIN_GROUP_SIZE = 2
_LIST_CAP = 10

FEASIBILITY_SCOPE = (
    "Each row uses only isolates with both a readable result for that agent and "
    "a recorded lineage. Support is the share of them in lineages of at least two "
    "isolates; the share is scored on those isolates and the singletons are set "
    "aside. The rarer-outcome count is a warning threshold, not an estimator gate."
)
FEASIBILITY_HEADERS = ("Agent", "Retained", "Support", "Input failures", "Rarer outcome")
...
def _agent_feasibility(panel: pd.DataFrame, lineage: pd.Series) -> Dict[str, dict]:
    ...
    for name in panel:
        ...
        if group["n_informative_groups"] < MIN_REPEATED_LINEAGES:
            failures.append(f"fewer than {MIN_REPEATED_LINEAGES} lineages with at least 2 isolates")
...
def group_adequacy(lineage: pd.Series) -> dict:
    ...
    return {
        ...
        "support": support,
        "estimable": bool(int(informative.size) >= MIN_REPEATED_LINEAGES),
...
def render_markdown(qc: dict) -> str:
    ...
    if reading:
        ...
        unreadable = list((reading.get("unrecognized_calls") or {}).items())
        if unreadable:
            lines.append(
                "Values that could not be read as a call: "
                + ", ".join(f"`{value}` ({n})" for value, n in unreadable[:_LIST_CAP])
                + (" and others" if len(unreadable) > _LIST_CAP else "")
                + f". The declared vocabulary is `{reading['phenotype_kind']}`.")
    if "mic_join" in qc:
        j = qc["mic_join"]
    ...
    if "lineage" in qc:
        ...
        lines.append(
            ...
            f"(support): {_pct(g['support'])}; the clonal share is scored on these "
            "isolates and the singletons are set aside. "
            + ("The clonal share can be estimated on this cohort."
               if g["estimable"] else
               "The clonal share needs at least two lineages with at least two "
               "isolates; there is no between-lineage comparison on this cohort."))
