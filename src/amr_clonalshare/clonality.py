...
def _label_availability(y_a: np.ndarray, labelled_a: np.ndarray,
    ...
    ``labels_representative`` is retained as a deprecated conservative alias.
    It is true only for complete outcome and label data. The matching
    ``collection_generalization_supported`` concerns only the supplied finite
    collections and never a broader population.
    """
    ...
    return {
        ...
        "labels_representative": bool(complete),
