...
overfitted before it is asked anything. ``benchmarks/decomposition_vs_regression.py``
...
submission-driven collection is always. Neither is the true one, so a report
...
#: Why the label-availability gate refuses a non-binary trait that has
#: unlabelled isolates. Both of its tests are Fisher exact, so on such a trait
#: the labelled-versus-unlabelled comparison is never made and an untested
#: gate must not read as a cleared one.
_UNASSESSABLE_NOTE = ("the labelled-versus-unlabelled comparison cannot be "
                      "made on a non-binary trait")
...
#: not estimable.
#:
#: The value is read off the coverage curve in
#: ``benchmarks/decomposition_calibration.py``. Across 720 scenario cells and
#: 720,000 simulated decompositions there, coverage of the nominal 95 %
...
def _label_availability(y_a: np.ndarray, keep_a: np.ndarray,
    ...
    * **the trait differs between labelled and unlabelled isolates.** If
    ...
    rule that does not apply to it, and, where either collection holds an
    unlabelled isolate, refused rather than passed: every per-side p is then
    None, so the second condition could never fire and the gate would clear a
    cohort whose unlabelled isolates carry the whole of the difference.
    """
    ...
    unlabelled_present = bool((~keep_a & np.isfinite(y_a)).any()
                              or (~keep_b & np.isfinite(y_b)).any())
    unassessable = bool(not binary and unlabelled_present)
    representative = not (differential and informative) and not unassessable
    ...
    if unassessable:
        note = (note + "; " + _UNASSESSABLE_NOTE + ", so the gate refuses "
                "rather than clearing a comparison it never made")
    return {
...
def decompose_prevalence_difference(
    ...
    *,
    ...
    return {
        ...
        "composition_estimable": bool(availability["labels_representative"]),
        "not_estimable_because": sorted(
            ...
            + ([] if availability["labels_representative"]
               else [_UNASSESSABLE_NOTE] if not availability["assessable"]
        ...
        "convention": "a lineage absent from one collection is given that "
                      ...
                      "so independent of the convention, and places the whole "
...
def decompose_panel(
    ...
    *,
    ...
    for key, label in (("composition", "composition"),
        ...
        # A within-lineage component the shared-support gate refused is not
        # identified, so it cannot be a discovery whatever its p-value; and
        # where the lineage labels are missing unevenly between the two
        # collections in association with the trait, the labelled subsets
        # are not the collections, so neither component is one. The q-value
        # is still written so the reader can see what the gate withheld.
        admitted = np.array([
            (key != "within_lineage"
    ...
    # The component p-values are bootstrap p-values with a floor of
    # 2 / (draws + 1) taken over the draws that came back finite, so the
    # smallest q the step-up can return in a family of m agents is the worst
    # of those floors times m times the m-th harmonic number. Where it exceeds
    # q no discovery is possible at this budget, whatever the effect, and the
    # record says so rather than reporting an absence.
    m = len(usable)
    realised = [int(per_agent[a].get("n_boot_finite") or 0) for a in usable]
    n_finite = min(realised) if realised else 0
    if m and n_finite:
        floor_q = float(2.0 / (n_finite + 1) * m * sum(1.0 / k for k in range(1, m + 1)))
        family["smallest_attainable_q"] = floor_q
        if floor_q > float(q):
            family["warning"] = (
                f"with {n_finite} finite draw(s) of the requested "
                f"n_boot={int(n_boot)} the smallest q the step-up can return "
                f"over {m} agents is {floor_q:.3f}, above the target {float(q):g}; "
                f"raise surveillance.n_boot before reading an absence of discoveries")
...
def trait_concentration(
    ...
    *,
    ...
    when ``L`` lineages hold it equally. The reading is the one an effective
    number of independent agents has in the decomposition family: a
    ...
    **The departure is a magnitude and says nothing about which way.** It is
    ...
    """
    if int(n_perm) < 1:
        raise ValueError(f"n_perm={n_perm}; the direction and the p-value are "
                         "read against a permutation null, which needs at "
...
def _concentration_null(codes: np.ndarray, w: np.ndarray, n_iso: int,
    ...
    """
    out = np.empty((n_perm, 2), dtype=float)
