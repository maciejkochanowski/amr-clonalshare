...
The names ``e_process``, ``test supermartingale`` and ``anytime_valid`` are used in
...
convenience here: an unshrunk group of one training isolate can give a probability
of zero or one and hence assign zero likelihood to a possible held-out outcome.
The fitted probabilities are therefore kept inside [1e-6, 1 - 1e-6], including
the grand-mean fallback, so that no batch can drive a running product to zero
for good; a zero e-value would be valid, but it ends the programme's evidence.
...
collection, they are multiplied, which forms a test supermartingale and is what
...
recomputed values is not a test supermartingale, so no guarantee attaches to the
...
def _shrunk_group_p(y_tr: np.ndarray, code_tr: np.ndarray, G: int):
    ...
    if gp < 2 or n - gp < 1:
        # Too few training rows to separate the two variance components:
        # every group gets the grand mean, clipped away from 0 and 1 so that
        # a first batch of identical outcomes cannot make a later outcome
        # impossible and drive the running product to zero for good.
        return np.full(G, float(np.clip(grand, 1e-6, 1 - 1e-6))), grand
...
def _log_bernoulli(y: np.ndarray, p: np.ndarray) -> float:
    # 0*log(0)=0, while an impossible outcome has log likelihood -inf.
    # Clipping the null p would lower its supremum and invalidate the ratio.
    return float((xlogy(y, p) + xlog1py(1.0 - y, -p)).sum())


def e_process(y, lineage, *, folds: int = 5, repeats: int = 20,
              seed=0) -> EResult:
    """Fixed-look evidence that binary ``y`` depends on ``lineage``.
    ...
    The null is a common-probability independent Bernoulli law conditional on
    the split and labels. Finite values other than 0 or 1 raise; non-finite
    outcomes and untyped labels are set aside. The value may be zero or below
    one and can grow without bound. No permutation or asymptotic approximation
    is used. For repeated looks use :func:`sequential_e_process`.
    """
    ...
    if np.any(np.isfinite(y) & (y != 0) & (y != 1)):
        raise ValueError("evidence requires binary outcomes (0 or 1); non-finite values are set aside")
    ...
    # Compute the average on the log scale, including the all-zero case.
    # Natural-scale overflow/underflow must not erase the log evidence.
    log_e = float(logsumexp(logs) - np.log(logs.size))
    with np.errstate(over="ignore", under="ignore"):
        e = float(np.exp(log_e))
    return EResult(e_value=e, log_e=log_e,
...
class SequentialEResult:
    ...
    n_batches, n_per_batch : the programme the evidence came from, after
        ...
        ``n_dropped_untyped``).
    n_analyzed_per_batch : isolates with both a finite outcome and a typed
        label in each batch. Unlike the archived ``n_per_batch`` definition,
        this excludes unread outcomes as well as untyped labels.
    n_scored_batches : batches passing the training-size, training-lineage
        and nonempty held-out conditions. Zero means the returned initial
        wealth of one has not scored any data.
    ...
    n_dropped_untyped: int = 0
    n_dropped_non_finite: int = 0
    n_analyzed_per_batch: List[int] = field(default_factory=list)
    n_scored_batches: int = 0
...
def sequential_e_process(batches, lineage_batches, *,
    ...
    """Anytime-valid binary-outcome evidence over a programme of batches.
    ...
    Each null batch has conditionally independent Bernoulli outcomes with a
    common probability, given the past and prespecified design. Finite values
    other than 0 or 1 raise; non-finite outcomes and untyped labels are set aside.
    """
    ...
    for t, (y, lab) in enumerate(zip(ys, ls)):
        ...
        if np.any(np.isfinite(y) & (y != 0) & (y != 1)):
            raise ValueError(f"batch {t}: evidence requires binary outcomes (0 or 1); non-finite values are set aside")
    ...
    analyzed_sizes = [int(np.isfinite(y).sum()) for y in ys]
    scored_batches = 0
    ...
    return SequentialEResult(
        ...
        n_dropped_untyped=n_dropped_untyped,
        n_dropped_non_finite=n_dropped_non_finite,
        n_analyzed_per_batch=analyzed_sizes,
        n_scored_batches=scored_batches)


def combine_independent(values: Iterable[float]) -> float:
    """Multiply e-values from independent batches without intermediate overflow.

    A factor below one can reduce accumulated evidence. Conditional validity,
    not a calendar label or disjoint file name, is required for sequential use.
    A zero multiplied by a genuinely infinite input is undefined and rejected.
    """
    v = np.asarray(list(values), dtype=float)
    if np.isnan(v).any() or (v < 0).any():
        raise ValueError("e-values must be non-negative and not NaN")
    if np.isinf(v).any() and (v == 0).any():
        raise ValueError("an infinite e-value multiplied by a zero one is undefined")
    if (v == 0).any():
        return 0.0
    if np.isinf(v).any():
        return float("inf")
    import math
    total = math.fsum(float(x) for x in np.log(v))
    with np.errstate(over="ignore", under="ignore"):
...
def combine_within_cohort(values: Iterable[float]) -> float:
    ...
    not."""
    v = np.asarray(list(values), dtype=float)
    if v.size == 0 or np.isnan(v).any() or np.any(v < 0):
        raise ValueError("e-values must be non-negative, non-empty and not NaN")
    maximum = float(v.max())
    if maximum == 0 or np.isinf(maximum):
        return maximum
    return float(maximum * np.mean(v/maximum))
