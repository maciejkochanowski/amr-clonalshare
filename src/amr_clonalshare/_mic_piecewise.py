    ...
    for order in (12, 24, 48, 96, 192):
        ...
        if previous is not None:
            ...
            if errors[0] <= tolerance and relative_score <= tolerance:
                return value, float(max(errors[0], relative_score))
        previous = value
...
def integrate_lineages(problem, counts, modes, scales, mean, sigma, tau2, *, strict=False):
    """Return log likelihood and mean/log-sigma/log-tau scores with error check."""
    total = np.zeros(4)
    error = 0.
    for nn, mode, scale in zip(counts, modes, scales):
        present = nn > 0
        value, local_error = _group(problem.a[present], problem.b[present], nn[present],
            problem.exact[present], mode, scale, mean, sigma, tau2, strict)
        total += value
        error += local_error
    return (*map(float, total), float(error))
