...
collected 2002 to 2021; 85 serovars, 534 SNP clusters. The two
configurations beside this file read it at the two typing resolutions the
release carries.
...
The nalidixic-acid reading uses the 6,915 of these isolates
...
| `serovar` | the text after "serovar" in the release's organism name, as written; antigenic formulae such as `4,[5],12:i:-` are kept as they are, and `-:r:1,5` is not merged into Infantis here |
| `pds_cluster` | the SNP cluster accession of the release |
| `collection_year` | the first four characters of the collection date |
| `period` | `before_2016` or `from_2016`, a cut fixed in advance on the 2014 detection of the emergent Infantis clone in United States retail meat |
...
SNP cluster, with five folds, ten repeats, 300 bootstrap draws and 150
permutations. The expected outputs of both are in `expected_serovar/` and
`expected_cluster/`. For nalidixic acid the serovar run returns
0.766 (0.033 to 0.853) and the cluster run 0.936 (0.237 to 0.977), with the permuted
control near zero at both; the exact realised interval is withheld on
nalidixic acid and on most of the panel, because a call at a few per cent
prevalence has within-lineage residuals far heavier-tailed than the Gaussian
law that interval assumes, and opens only on the agents whose prevalence sits
nearer one half; the report says which.
...
Kitagawa split between the periods. Neither is part of the reported reading
