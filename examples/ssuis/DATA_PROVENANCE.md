...
**Coding is `0` = wild type, `1` = non-wild-type**, written `WT` and `NWT` in
`data/calls_long.csv` and declared `phenotype_kind: wt_nwt` in `config.yaml`.

**Three of the thirteen agents have a EUCAST epidemiological cut-off (ECOFF)
and ten do not.** Tetracycline (4 mg/L) is a EUCAST ECOFF; doxycycline (0.5)
and erythromycin (0.25) are tentative EUCAST ECOFFs, shown in parentheses on
the EUCAST MIC distribution website for *S. suis* (species 508). EUCAST sets no
value for tiamulin, tilmicosin or tylosin and lists lincomycin as having
insufficient data. For the ten the cut-off is a collection-specific cut-off:
a log-normal is fitted iteratively to the wild-type part of this collection's
MIC distribution, as ECOFFinder does, and the 99th percentile is rounded up to
the next two-fold dilution (Turnidge, Kahlmeter & Kronvall 2006, *Clin
Microbiol Infect* 12:418-425). It is not an ECOFF: EUCAST sets an ECOFF from
distributions pooled across several laboratories, and one collection read in
two laboratories cannot stand in for that. The readings are CLSI broth
microdilution, whereas EUCAST distributions rest on EUCAST and ISO 20776-1
methods. `derive_ecoffs.py` retrieves the published values, fits the others
and writes `data/ecoff_derived.json`.

| agent | cut-off (mg/L) | source | non-wild-type |
|---|---|---|---|
| doxycycline | 0.5 | EUCAST, tentative | 571 (84.3 %) |
| tetracycline | 4 | EUCAST | 574 (84.8 %) |
| erythromycin | 0.25 | EUCAST, tentative | 366 (54.1 %) |
...
| penicillin | 0.06 | derived | 156 (23.0 %) |
| tiamulin | 8 | derived | 131 (19.4 %) |
| spectinomycin | 32 | derived | 78 (11.5 %) |
| amoxicillin | 0.06 | derived | 37 (5.5 %) |
| cefquinome | 0.125 | derived | 26 (3.8 %) |

The cut-offs are written on the conventional dilution series (0.06 for the
0.0625 mg/L well). A derived cut-off is specific to the collection it was
derived from and is not proposed as a species-level value; any analysis that
turns on the ten derived agents inherits that, which is why the three agents
with a EUCAST value can be reported separately. Two derived values are limited
by the lowest well of the panel: 551 of 628 amoxicillin readings and 444 of 628
penicillin readings from the Fordham laboratory sit on its lowest well
(0.03 mg/L), so the fitted wild type is truncated from below and the cut-off of
0.06 mg/L is one dilution above that floor.

### How the fitted values compare with EUCAST

Where a EUCAST value exists it is used. The fit was also run on those agents
and compared with it, which checks the fitting code, not the ten derived
values. Across the four agents in the full 16-agent panel that have one:
...
Florfenicol agrees trivially, since no isolate is non-wild-type; erythromycin
differs by two dilutions. Parenthesised values are tentative EUCAST cut-offs.
...
cut-off. Every one of the 10,832 MIC records in this cohort is a value on a
tested well with no censoring operator recorded, so the call needs no
interval rule; the dilution reading of `censored.py` treats a value on the
lowest or highest well of an agent's panel as censored, an assumption it
declares through `panel_geometry` and brackets in its sensitivity arm.

The panel was read in two laboratories. The Methods of the source study
place the British and Canadian isolates at LGC, Fordham, UK, and the
Vietnamese isolates at the Oxford University Clinical Research Unit, Ho Chi
Minh City, and the two tested different dilution ranges: for amoxicillin the
Vietnamese readings reach 0.015 mg/L where the others stop at 0.03 mg/L.
Read on the pooled lattice, the 551 readings of 0.03 mg/L from Fordham would
pass as an interior dilution and the left-censoring of 81 % of the cohort
would be lost. `build_mic_table.py` therefore writes a `testing_laboratory`
column from the country of isolation, and `config.yaml` names it as
`mic_panel_column`, so that the end wells are found within each laboratory.
The laboratories' own panel specifications were not available; the wells are
inferred from the recorded values one laboratory at a time, and the laboratory
is assigned from the country as in the source study. That assignment is an
assumption: the source study says some of its MICs had been published before,
and 27 British isolates of 2010-2011 read marbofloxacin at 0.015 mg/L with no
reading between that and 0.25 mg/L, which suggests a third panel.
`config.yaml` also names `isolation_country` as `stratify_by`, so the shipped
record carries, beside the pooled run, the same analyses within each of the
three countries (`strata` in the record, `strata_results.csv` beside it).
...
trailing zero and silently fails to join.
...
serotype field. The shipped configurations read the population cluster and
the sequence type as the lineage, never `serovar`; the serotype reading of
the cohort (`benchmarks/ssuis_resolution.py`) is the one the repair affects,
and it affects it by returning 39 isolates to the serotype they carry.
...
and it is attached by `link_source_lineages.py`. It gives 30 clusters against
...
estimability gate is for. The worked analyses use `baps_cluster`.
...
## 7. The determinant calls

The source study reports presence or absence of 43 resistance determinants,
called from the genome assemblies of the same isolates, in
`data/source_table_s1.csv`. They are used in one place only,
`benchmarks/ssuis_mechanism.py`, and never in the phenotype analysis. They are
an independent measurement of the same isolates by a different instrument,
which is what makes them a check on the lineage share rather than an
illustration of it.
...
| `data/mic_panel.csv`, `data/mic_panel_receipt.json` | the MIC table `config.yaml` reads, with the testing laboratory of every reading, and the receipt `build_mic_table.py` wrote beside it |
...
| `data/calls_long.csv` | the same 8,801 calls in long form, one row per isolate and agent, the table `config.yaml` reads; written by `build_calls_long.py` and checked against the two files above by `tests/test_ssuis_provenance.py`. written `NWT` and `WT`: a non-wild-type or wild-type call against the cut-off in `ecoff_derived.json` |
...
| `expected/` | the record, the two reports and the input check the shipped run of `config.yaml` wrote at seed 42 |
| `config.yaml`, `config_contrast.yaml` | the shipped run, and the same cohort read at sequence-type resolution with the two countries declared as the collections to contrast (section 6); the second has no shipped output |
| `build_calls_long.py` | melts the two wide files into `data/calls_long.csv` |
