...
is why the three published agents can be reported separately as a
...
still inferred from the recorded values, now one laboratory at a time.
...
place only, `validate_with_determinants.py`, and never in the phenotype
analysis. They are an independent measurement of the same
...
| `data/calls_long.csv` | the same 8,801 calls in long form, one row per isolate and agent, the table `config.yaml` reads; written by `build_calls_long.py` and checked against the two files above by `tests/test_ssuis_provenance.py`. The words `non-susceptible` and `susceptible` are the reader's vocabulary for 1 and 0; what was measured is a non-wild-type call against the cut-off in `ecoff_derived.json` |
...
| `validate_with_determinants.py` | the same decomposition on the determinant calls |
