...
def main() -> int:
    ...
    refused = [a for a in agents if per_agent[a]["status"] == "ok"
               and not per_agent[a]["within_lineage_estimable"]]
    if refused:
        reasons = sorted({r for a in refused
                          for r in per_agent[a]["not_estimable_because"]})
        print(f"  within-lineage component REFUSED for {len(refused)} of "
              f"{len(agents)} agents: {'; '.join(reasons)}")
    refused_comp = [a for a in agents if per_agent[a]["status"] == "ok"
                    and not per_agent[a]["composition_estimable"]]
    if refused_comp:
        print(f"  composition component REFUSED as well for "
              f"{len(refused_comp)} of {len(agents)} agents: the lineage "
              f"labels are differentially missing in association with the trait")
    if family.get("warning"):
        print(f"  WARNING: {family['warning']}")
