"""Cut-offs for the S. suis panel: EUCAST (T)ECOFFs where they exist,
collection-specific cut-offs elsewhere.

Four of the sixteen agents have a published EUCAST (T)ECOFF. For the other
twelve a collection-specific cut-off is fitted to this collection's MIC
distribution as ECOFFinder does: iterative fitting of a log-normal to the
wild-type subpopulation, Turnidge, Kahlmeter & Kronvall 2006, Clin Microbiol
Infect 12:418-425. It is not an ECOFF, which EUCAST sets from distributions
pooled across laboratories.
...
On the four agents that have a published EUCAST value the fitted cut-off is
also computed and compared with it, which checks the fitting code; it does not
make the twelve fitted values ECOFFs.
"""
import csv
import html
import json
