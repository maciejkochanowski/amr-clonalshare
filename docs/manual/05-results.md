...
| MIC | Dilution-scale component with its calibrated 95% interval (`calibrated`), the older moment estimate with its approximate F interval (`share`), panel geometry and the dilution table |
...
The result reader accepts historical schema 1.0 and current schema 2.0. Missing historical fields remain absent; reading an old record does not create a current analysis. Keep original records and manifests when migrating a downstream reader.
