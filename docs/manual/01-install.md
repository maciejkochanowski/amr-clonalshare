...
The package needs `numpy`, `scipy`, `pandas` and `pyyaml`, which `pip`
resolves; nothing else is imported at run time. The `dev` extra adds the
test, lint and build tools for a working copy of the repository.

```bash
python -m pip install "amr-clonalshare[dev]"
```
