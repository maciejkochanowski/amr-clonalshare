...
The calibrated interval costs whole processor-hours, because every tested value of rho is refitted on every simulated dataset. The records of this release time the shipped 16-agent *S. suis* panel at 199 draws on 16 processors: 7 to 37 minutes of wall-clock time per agent, a median of 23 minutes, 5.4 hours for the panel and 87 processor-hours in total (`benchmarks/results_mic_release/mic_empirical/*.json` records `elapsed_seconds` per agent). Fewer processors lengthen the wall-clock time roughly in proportion. Plan the run before switching the calibrated interval on for a wide panel, or use `screening_n_boot` to find the agents worth the full budget.
