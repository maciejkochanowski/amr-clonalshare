...
The Python entry point accepts the same loaded configuration and supports `results_dir`, `seed`, `overwrite=False` and an optional `progress` callback. Preserve the configuration, software version, seed, input files and manifest with the output. The validation results have their own recorded seeds and numerical budgets; the small recipes use deliberately modest demonstration budgets.
...
`calibration_n_boot: 199` resolves the p-value in steps of 0.005 and was the budget of the simulation study; it is the setting for routine reporting. For an interval that will be quoted, `999` narrows the Monte Carlo error of the limits at five to eight times the run time; the limits themselves are reported to a tolerance of 0.002 in rho and inherit the Monte Carlo error of the test at each tested value. Results do not depend on `workers`.

The calibrated interval costs whole processor-hours, because every tested value of rho is refitted on every simulated dataset. The records of this release time the shipped 16-agent *S. suis* panel at 199 draws on 16 processors: 9 to 30 minutes of wall-clock time per agent, a median of 19 minutes, 5.0 hours for the panel and 80 processor-hours in total (`benchmarks/results_mic_release/mic_empirical/*.json` records `elapsed_seconds` per agent). Fewer processors lengthen the wall-clock time roughly in proportion. Plan the run before switching the calibrated interval on for a wide panel, or use `screening_n_boot` to find the agents worth the full budget.
