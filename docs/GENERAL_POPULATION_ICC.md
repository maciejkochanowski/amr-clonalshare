...
Software 1.0.0 includes the general numerical method, which passed a
finite-panel empirical confirmation of its coverage target.
The general method computes for the actual retained group sizes. It does not
require an exact registered calibration bank. Existing analyses keep their
previous defaults unless this method is explicitly enabled.
...
With two or more repeated groups, the selected fitted-nuisance bootstrap uses
...
all retained components are diagnostics. Acceptance of the finite confirmation
panel does not establish an exact fitted-nuisance or continuous-parameter guarantee.
...
not remove or replace them while workers may run. Relative cache paths use the process working
directory; an absolute path is preferable for scheduled jobs.
...
workspace. At the selected settings, the 4,999 by 2,091 score matrix alone uses
...
process overhead, plus larger numerical caches left by earlier unrelated
computations, remain outside the estimate. Use fresh worker processes and
explicit scheduler memory limits for large jobs. Memory limits never change
the selected reference count, threshold or statistical grids.
