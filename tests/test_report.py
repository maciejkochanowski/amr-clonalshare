...
def test_the_page_is_self_contained_and_carries_the_same_numbers(planted):
    ...
    order = ("Measurement summary", "Admissibility of the input",
             ...
             "change within them", "Provenance and terms")
...
def test_formatting_never_invents_a_number():
    from amr_clonalshare.report_model import evalue, interval, num, pct, text
    assert num(None) == "not computed" and num(float("nan")) == "not computed"
    assert num(0.12345) == "0.123" and text(None) == "not recorded" and text(7) == "7"
    assert pct(None) == "not computed" and pct(0.5) == "50.0 %"
...
def test_the_page_draws_one_figure_per_question_the_record_can_answer():
    ...
    # the heat map of readings per lineage and dilution well is the sixth figure
    assert "Readings per lineage and dilution interval" in html
