...
def api():
    return importlib.import_module('amr_clonalshare._general_probit.histogram_table.histogram_table')
...
def test_boundary_entries_and_uniform_large_group_reference():
    ...
    for m in (1,2,683):
        ...
        for j,(a,rho) in enumerate(theta):
            expected=numerics.selected_count_log_probabilities(m,a,rho,tuple(range(m+1)))
...
def test_masked_batch_likelihood_matches_profile_likelihood(backend):
    ...
    expected=np.array([[-numerics.ProfileLikelihood(k,sizes).nll(a,rho) for a,rho in theta] for k in counts])
