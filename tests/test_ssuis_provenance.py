...
def test_the_long_panel_is_the_two_layers_and_nothing_else():
    """`calls_long.csv` carries the shipped calls in the shape a run with no
    genotype layers reads, and adds nothing to them."""
    import sys
    sys.path.insert(0, str(DATA.parents[2]))
    from examples.ssuis.build_calls_long import build

    shipped = pd.read_csv(DATA / "calls_long.csv", dtype={"genome_id": str})
    rebuilt = build()
    assert len(shipped) == 8801
    pd.testing.assert_frame_equal(
        shipped.sort_values(["genome_id", "antibiotic"]).reset_index(drop=True),
        rebuilt.sort_values(["genome_id", "antibiotic"]).reset_index(drop=True))
