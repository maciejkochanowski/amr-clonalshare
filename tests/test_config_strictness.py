...
@pytest.mark.parametrize("section,typo,intended", [
    ...
    ("dataset", "batch_colum", "batch_column"),
    ("evidence", "alpah", "alpha"),
...
def test_an_unknown_section_is_refused(raw):
    bad = copy.deepcopy(raw)
    bad["scoring"] = {"weights": [1.0]}
    with pytest.raises(ConfigError, match="unknown key 'scoring'"):
        from_dict(bad)
