...
def test_domain_diagnostics_do_not_claim_a_continuous_validation_region():
    ...
    assert r.domain_diagnostics["tested_group_count_range"] == [10, 100]
    assert r.domain_diagnostics["tested_max_group_size"] == 1000
...
def test_enable_does_not_change_any_classical_result_or_rng_stream(share_cfg):
    ...
    assert off["config"]["population_probit"] == {"enabled": False, "interval_method": "fixed_v3"}
    assert on["config"]["population_probit"] == {"enabled": True, "interval_method": "fixed_v3"}
...
def test_frozen_numerical_source_and_packaged_evidence_match():
    module = api()
    ...
    assert module._evidence()["tested_group_count_range"] == [10, 100]
