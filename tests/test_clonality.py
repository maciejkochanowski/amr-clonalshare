...
def test_the_gate_fires_when_missingness_is_both_differential_and_informative():
    ...
    assert r["within_lineage_estimable"] is False
    # The labelled isolates are then not the collections, so the composition
    # component is refused with it; shared support alone leaves it standing.
    assert r["composition_estimable"] is False
...
def test_a_non_binary_trait_with_unlabelled_isolates_is_refused_not_cleared():
    # The failure of the module docstring on a trait the Fisher test cannot
    # read: 5 % of one collection is unlabelled against 75 % of the other, and
    # the unlabelled isolates carry the high values. Both tests are Fisher
    # exact, so every per-trait p is None, missingness_informative is False and
    # the gate used to clear a comparison it had never made.
    ...
    assert r["within_lineage_estimable"] is False
    assert r["composition_estimable"] is False
    assert any("non-binary trait" in reason
               for reason in r["not_estimable_because"])
...
def test_carriage_concentration_refuses_a_null_of_zero_permutations():
    """The direction and the p-value are read against the permutation null,
    so there is nothing to read them against when none is drawn."""
    y = np.array([1, 0, 1, 0, 1, 0], dtype=float)
    lin = np.array(["a", "a", "b", "b", "c", "c"])
    with pytest.raises(ValueError, match="n_perm"):
        trait_concentration(y, lin, n_perm=0, rng=np.random.default_rng(0))
