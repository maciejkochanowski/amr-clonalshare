...
reader can check without reading the source.
"""
import numpy as np
import pytest

from amr_clonalshare.censored import (_trunc_moments,CENSORED_GROUP_LIMIT,
                                         SINGLE_CUT_PREVALENCE,
                                         censored_clonal_share,
                                         intervals_from_binary,
                                         intervals_by_panel, intervals_from_mic, panel_geometry,
                                         scale_is_identified)
...
def test_zero_width_intervals_reproduce_the_closed_form():
    ...
    got = censored_clonal_share(z, z, lineage).kappa
...
def test_unbalanced_design_beats_the_moment_estimator():
    ...
    for _ in range(40):
        ...
        ours.append(censored_clonal_share(z, z, lineage).kappa)
...
def test_dichotomising_a_mic_gives_the_binary_constructor_intervals():
    ...
    a = censored_clonal_share(*from_call, lineage).kappa
    b = censored_clonal_share(*by_hand, lineage).kappa
...
def test_the_ladder_recovers_one_latent_share_from_three_readings():
    ...
    for _ in range(12):
        ...
        point.append(censored_clonal_share(z, z, lineage).kappa)
        ...
        hi = np.where(idx < wells.size, wells[np.clip(idx, 0, wells.size - 1)],
                      np.inf)
        interval.append(censored_clonal_share(lo, hi, lineage).kappa)
        blo, bhi = intervals_from_binary((z > 1.0).astype(float), cutoff_log2=1.0)
        binary.append(censored_clonal_share(blo, bhi, lineage).kappa)
...
def test_share_is_near_zero_when_lineage_carries_nothing():
    ...
    lineage = np.array(["L%d" % g for g in code], dtype=object)
    assert censored_clonal_share(z, z, lineage).kappa < 0.06
...
def test_binary_result_says_the_scale_is_a_convention():
    ...
    res = censored_clonal_share(lo, hi, lineage)
...
def test_lineage_length_mismatch_is_refused():
    ...
    with pytest.raises(ValueError):
        censored_clonal_share(lo, hi, ["A"] * 9)
...
def test_too_few_lineages_is_refused_rather_than_estimated():
    ...
    res = censored_clonal_share(z, z, ["A"] * 40)
...
def test_the_reported_interval_brackets_the_point_estimate():
    ...
    z, _code, lineage = _latent(rng, [20] * 25)
    res = censored_clonal_share(z, z, lineage)
...
def test_the_interval_widens_as_the_reading_coarsens():
    ...
    z, _code, lineage = _latent(rng, [25] * 30)
    exact = censored_clonal_share(z, z, lineage)
    lo, hi = intervals_from_binary((z > 0.0).astype(float))
    call = censored_clonal_share(lo, hi, lineage)
...
def test_a_cut_in_a_tail_is_refused_and_the_dilution_is_not():
    ...
    assert "outside the reporting window" in call.reason
    assert "calibrated window" not in call.reason
    wells = np.arange(-4.0, 6.0)
    idx = np.searchsorted(wells, z, side="left")
    dlo = np.where(idx > 0, wells[np.clip(idx - 1, 0, wells.size - 1)], -np.inf)
    dhi = np.where(idx < wells.size, wells[np.clip(idx, 0, wells.size - 1)],
                   np.inf)
    dilution = censored_clonal_share(dlo, dhi, lineage)
...
def test_a_cut_inside_the_window_is_not_refused_for_prevalence():
    ...
    assert "outside the reporting window" not in res.reason
...
def test_the_gate_names_the_quantity_it_read():
    rng = np.random.default_rng(101)
    lo, hi, lineage = _left_censored(rng, cut=3.0)
    reason = censored_clonal_share(lo, hi, lineage).reason
    assert "%" in reason and "wholly beyond the panel" in reason


def test_truncated_moments_hold_in_both_tails():
    """The conditional mean of a normal restricted to an interval eight
    residual standard deviations above the mean is above eight, not the mean
    itself: log_ndtr saturates for large positive arguments, so an interval
    in the upper tail is reflected before the difference of logarithms is
    formed. Checked against scipy's truncated normal in both tails and across
    the middle."""
    from scipy.stats import truncnorm
    cases = [(8.0, np.inf), (-np.inf, -8.0), (8.0, 9.0), (-9.0, -8.0),
             (30.0, np.inf), (-1.0, 1.0), (0.0, np.inf), (7.0, np.inf),
             (2.5, 4.0), (-4.0, -2.5)]
    for a, b in cases:
        mean, var = _trunc_moments(np.array([a]), np.array([b]), 0.0, 1.0)
        law = truncnorm(a, b)
        assert mean[0] == pytest.approx(law.mean(), abs=1e-8)
        assert var[0] == pytest.approx(law.var(), abs=1e-8)
    mean, var = _trunc_moments(np.array([7.0]), np.array([np.inf]), 3.0, 0.5)
    law = truncnorm(8.0, np.inf, loc=3.0, scale=0.5)
    assert mean[0] == pytest.approx(law.mean(), abs=1e-8)


def test_untyped_isolates_are_set_aside_and_counted():
    rng = np.random.default_rng(8092026)
    v = np.r_[rng.normal(-2, .2, 30), rng.normal(2, .2, 30)]
    lab = np.array([None] * 30 + ["A"] * 30, dtype=object)
    r = censored_clonal_share(v, v, lab)
    # thirty untyped isolates are not a second lineage: one lineage remains
    assert r.n_dropped_untyped == 30
    assert r.n == 30 and r.n_groups == 1
    assert not r.estimable


def test_an_interval_with_lo_above_hi_is_refused_as_input():
    with pytest.raises(ValueError, match="lo > hi"):
        censored_clonal_share(np.ones(40), np.zeros(40),
                              np.repeat(np.arange(4), 10))


def test_readings_that_are_all_one_value_carry_no_variance():
    r = censored_clonal_share(np.ones(40), np.ones(40),
                              np.repeat(np.arange(4), 10))
    assert not r.estimable and "no variance" in r.reason
