...
def test_a_run_without_metadata_is_refused():
    with pytest.raises(ConfigError, match="metadata"):
        _dataset(metadata=None, lineage_column=None).validate()


def test_a_run_without_a_lineage_column_is_refused():
    with pytest.raises(ConfigError, match="lineage_column"):
        _dataset(lineage_column=None).validate()


def test_a_run_without_a_phenotype_or_a_dilution_table_is_refused():
    with pytest.raises(ConfigError, match="phenotype"):
        _dataset(phenotype=None).validate()
    _dataset(phenotype=None, mic="mic.csv").validate()
...
def _decomp(diff, comp, within, comp_ci, within_ci, estimable=True):
    # a discovery is the step-up's verdict, which in these fixtures is taken
    # to coincide with the interval excluding zero
    return {"status": "ok", "difference": diff, "composition": comp,
            ...
            "composition_discovery": comp_ci[0] * comp_ci[1] > 0,
            "within_lineage_discovery": within_ci[0] * within_ci[1] > 0,
            "within_lineage_estimable": estimable,
            "composition_estimable": True}
...
def test_offsetting_components_are_singled_out():
    ...
    assert out["n_composition_discoveries"] == 2
    assert out["n_within_lineage_discoveries"] == 1
    assert out["offsetting_features"] == ["flat"]


def test_a_refused_within_lineage_component_is_not_a_discovery():
    # the interval excludes zero but the shared-support gate refused the
    # component: the summary counts the refusal, not a rate change
    meta = {"prevalence_decomposition": {
        "contrast_column": "country", "levels": ["a", "b"],
        "per_feature": {"x": _decomp(0.10, 0.02, 0.08, [-0.05, 0.09],
                                     [0.03, 0.13], estimable=False)}}}
    out = _surveillance_summary(meta)["decomposition"]
    assert out["n_within_lineage_discoveries"] == 0
    assert out["n_within_lineage_refused"] == 1
    assert out["within_lineage_refused_features"] == ["x"]
...
# --- the programme of intakes ----------------------------------------------

def test_a_run_with_a_contrast_carries_the_decomposition(tmp_path):
    """The record has to carry the decomposition, not only the API: a
    programme that declares two collections reads the split from the run."""
    import pandas as pd

    from amr_clonalshare import core
    from conftest import planted_cohort, write_config

    raw = planted_cohort(tmp_path, intake=False)
    meta = pd.read_csv(tmp_path / "data" / "meta.csv", dtype=str)
    meta["side"] = ["early" if i % 2 else "late" for i in range(len(meta))]
    meta.to_csv(tmp_path / "data" / "meta.csv", index=False)
    raw["dataset"].update({"contrast_column": "side",
                           "contrast_levels": ["late", "early"]})
    raw["surveillance"] = {"n_boot": 50, "n_perm": 50}
    _, cfg = write_config(tmp_path, raw)

    r = core.run(cfg, seed=7)
    md = r["metadata_diagnostics"]
    dec = md["prevalence_decomposition"]
    assert dec["contrast_column"] == "side" and dec["levels"] == ["late", "early"]
    assert dec["n_a"] + dec["n_b"] == 96
    assert set(dec["per_feature"]) == {"agentA", "agentB", "agentC"}
    assert set(md["lineage_resolved_prevalence"]) == {"agentA", "agentB", "agentC"}
    assert set(md["trait_concentration"]) == {"agentA", "agentB", "agentC"}
