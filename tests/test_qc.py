...
def test_group_adequacy_matches_the_estimator_definition_of_support():
    ...
    assert g["estimable"] is False          # one lineage repeats, so no contrast
    assert qc.group_adequacy(pd.Series(["L0"] * 3 + ["L1"] * 2 + ["L2"]))["estimable"] is True
    n0 = (40 - (30 ** 2 + 10) / 40) / 10
...
def test_a_fine_lineage_definition_is_reported_as_not_estimable(tmp_path):
    ...
    assert "no between-lineage comparison" in text and "40 lineages hold a single isolate" in text
