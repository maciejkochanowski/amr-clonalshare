"""FDR, permutation p-values, effective dimension."""
import numpy as np
import pytest
...
def test_benjamini_hochberg_refuses_a_failed_test_rather_than_zeroing_the_family():
    """One NaN must not make every adjusted value NaN, every rejection False,
    and a family with three clear discoveries read as "nothing significant"
    with no warning. A failed upstream test is not a null result, so it stops
    the run."""
...
def test_effective_dimension_drops_isolates_with_a_missing_result_not_agents():
    rng = np.random.default_rng(4)
    X = rng.normal(size=(200, 5))
    full = effective_dimension(X)
    holes = X.copy(); holes[3, 0] = np.nan; holes[10, 4] = np.nan
    assert abs(effective_dimension(holes) - full) < 0.2
    assert np.isnan(effective_dimension(np.full((3, 2), np.nan)))
