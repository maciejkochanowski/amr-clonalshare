...
def test_unreadable_calls_are_accounted_for_in_input_qc(tmp_path):
    ...
    assert "3 unrecognized" in text
    assert "`unreadable` (3)" in text


def test_a_wholly_unreadable_call_table_stops_the_run_with_a_named_vocabulary(tmp_path, capsys):
    raw = planted_cohort(tmp_path)
    path = tmp_path / "data" / "calls.csv"
    table = pd.read_csv(path)
    table["call"] = np.where(table["call"].str.startswith("non"), "R", "S")
    table.to_csv(path, index=False)
    config = tmp_path / "config.yaml"
    config.write_text(yaml.safe_dump(raw), encoding="utf-8")
    assert main(["--config", str(config), "--check-input"]) == 2
    message = capsys.readouterr().err
    assert "no call in the phenotype table could be read" in message
    assert "clinical_sir" in message
...
def test_single_lineage_input_explains_the_missing_comparison():
    ...
    assert "fewer than 2 lineages with at least 2 isolates" in text
