...
def test_benjamini_hochberg_matches_the_step_up_definition(family, q):
    ...
    for i in range(m):
        ...
        if exact[i] != q:
            # On an exact tie the floating-point adjusted value can land a
            # unit in the last place either side of q, so the tie is not a
            # statement the test can make.
            assert bool(reject[i]) == (exact[i] < q)
...
def test_clonal_share_does_not_depend_on_what_the_lineages_are_called(seed, lin):
    ...
    for key in ra:
        if key in ("ci_low", "ci_high", "latent_low", "latent_high",
                   "latent_species_low", "latent_species_high"):
            # the latent endpoints are monotone images of the interval
            continue
        a, b = ra[key], rb[key]
