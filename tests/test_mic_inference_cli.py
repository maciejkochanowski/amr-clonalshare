...
def test_a_byte_order_mark_and_a_blank_value_are_read_or_named(tmp_path, capsys):
    source=tmp_path/'in.csv';output=tmp_path/'r.json';write_exact(source)
    source.write_bytes(b'\xef\xbb\xbf'+source.read_bytes())
    assert api().main([str(source),'--method','exact','--output',str(output)])==0
    blank=tmp_path/'blank.csv';blank.write_text('lo,hi,lineage\n0,0,A\n,1,A\n')
    assert api().main([str(blank),'--method','exact','--output',str(tmp_path/'b.json')])==2
    assert 'line 3: lo' in capsys.readouterr().err


def test_a_file_system_without_hard_links_still_gets_the_result(tmp_path, monkeypatch):
    import os
    source=tmp_path/'in.csv';output=tmp_path/'r.json';write_exact(source)
    ...
    assert api().main([str(source),'--method','exact','--output',str(output)])==0
    assert json.loads(output.read_text())['result']['method']=='exact_generalized_F'
