...
def test_frozen_sources_and_selected_contract():
    evidence = api._protocol()
    assert evidence['accepted'] is True
    assert evidence['internal_alpha'] == .04 and evidence['B'] == 4999
    assert evidence['validation_status'] == 'confirmation_accepted'
    assert evidence['confirmation_evidence']['confirmation_summary_sha256']
    assert hashlib.sha256(Path(kernel.__file__).read_bytes()).hexdigest() == '1aa4212eacee92e7aac38d1fabb5b61618203eafa0332211fcf9fc596843735b'
...
def test_general_config_default_serialization_and_grouped_filtering():
    ...
    assert core._config_record(cfg)['population_probit']=={'enabled':False, 'interval_method':'fixed_v3'}
...
def test_general_only_report_uses_object_and_real_counts(accepted, monkeypatch):
    protocol = dict(api._protocol(), accepted=accepted,
                    validation_status='confirmation_accepted' if accepted else 'confirmation_pending')
    ...
    assert ('confirmed the coverage target' in report) == accepted
    assert ('is not confirmed' in report) == (not accepted)
