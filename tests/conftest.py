"""Shared fixtures. Cohorts are kept small so the fast lane stays short."""
...
# A suite that explores different inputs on every run cannot be quoted. The
# property tests draw the same examples every time, so a failure is
# reproducible from the version alone, a mutation score is a number and not a
# distribution, and a new counterexample arrives when the strategy or the code
# changes rather than at random.
settings.register_profile("reproducible", derandomize=True,
...
def _fast_calibrated_mic_interval(request, monkeypatch):
    """The calibrated MIC interval costs minutes per agent; pipeline tests
    that do not examine it receive a recorded placeholder instead."""
    if "real_calibration" in request.keywords:
        return
    from amr_clonalshare import core
    monkeypatch.setattr(core, "_calibrated_mic_interval", lambda *args, **kwargs: {
        "estimable": False, "status": "not_computed", "reason": "skipped in fast tests"})
