"""The MIC likelihood, its gradient and the cases that break quadrature.

The gradient is checked against a central finite difference. Its
tolerance of 3e-5 is not a number chosen until the test passed: with a
step of about 1e-5 the truncation error of a central difference is of
order h^2 and its cancellation error of order eps/h, that is near 1e-6
relative, so a threshold an order above that separates a wrong
derivative from the difference scheme's own error.
"""
import importlib
import numpy as np
import pytest
...
def test_singleton_rich_optimizer_excursion_is_resolved():
    ...
    a,b,c,_,_=generate(design_grid()[32],0,stream=1)
...
def test_high_rho_profile_has_verified_quadrature():
    from benchmarks.mic_inference.calibrate import generate,design_grid
    a,b,c,_,_=generate(design_grid()[50],0,stream=1)
...
def test_asymmetric_panel_profiles_match_independent_integrator(cell,rep):
    from benchmarks.mic_inference.calibrate import generate,design_grid
    a,b,c,_,_=generate(design_grid()[cell],rep,stream=1)
