...
def config(tmp_path, call_text=None, mic_text=None, metadata='Strain_ID,ST\n001,A\n002,B\n'):
    (tmp_path / 'meta.csv').write_text(metadata)
...
def test_duplicate_policies_and_empty_agent_registry():
    ...
    legacy = to_non_susceptible(d, duplicate_policy='legacy')
    assert legacy.loc['000', 'a'] == 1
...
def test_legacy_vocabulary_matches_historical_readable_subset(intermediate):
    ...
    old_map = {'resistant': 1., 'susceptible': 0.,
               'intermediate': {'non_susceptible': 1., 'susceptible': 0., 'drop': np.nan}[intermediate],
               'nonsusceptible': 1., 'non-susceptible': 1.}
    expected = pd.Series(values).str.lower().map(old_map).to_numpy()
    with pytest.warns(RuntimeWarning, match='legacy'):
        p = to_non_susceptible(calls(values), kind='legacy', intermediate=intermediate, duplicate_policy='legacy')
