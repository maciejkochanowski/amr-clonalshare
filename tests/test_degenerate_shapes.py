...
def test_support_exactly_at_the_threshold_is_admitted():
    """Ten isolates, two of them alone in their lineages: support is 0.80,
    the threshold itself, and the gate is 'at least', not 'above'."""
    labels = ["A"] * 5 + ["B"] * 3 + ["C", "D"]
    adequacy = qc.group_adequacy(pd.Series(labels))
    assert adequacy["support"] == pytest.approx(0.8)
    assert adequacy["estimable"] is True
    labels = ["A"] * 5 + ["B"] * 2 + ["C", "D", "E"]
    below = qc.group_adequacy(pd.Series(labels))
    assert below["support"] == pytest.approx(0.7)
